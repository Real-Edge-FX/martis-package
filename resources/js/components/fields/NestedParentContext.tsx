 ...
 *
 * Relationship panels read it through `useRelationParent()`.
 */
 ...
 * NestedParent when the panel renders inside a card or a drawer, else the
 * record in the route of the detail or edit page
 * (`/resources/:resource/:id`). Every relationship panel resolves its parent
 * here, so none of them can fall back to the page record while nested.
 */
export function useRelationParent(): { resource: string; id: string } {
  ...
  if (nested) {
    return { resource: nested.resource, id: String(nested.id) }
  }
