...
function MorphManyDetailTable({ field }: { field: FieldDisplayProps['field'] }) {
  ...
  const recordsQuery = useQuery({
    queryKey: ['morph-many', parentResource, parentId, relationship, { search, page, perPage, sort, direction }],
    queryFn: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (relatedId: string | number) =>
      api.delete(`/api/resources/${parentResource}/${parentId}/morph-many/${relationship}/${relatedId}`),
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ['morph-many', parentResource, parentId, relationship] })
  ...
  return (
    <div className="mt-6 space-y-3">
          ...
          {meta?.searchable && (
            <div className="relative">
              ...
              <input
                ...
                className="morph-many-search-input w-full rounded-md border py-1.5 pl-8 pr-3 text-sm sm:w-auto"
                style={{
      ...
      <style>{`
        .morph-many-search-input::placeholder {
          color: var(--martis-text-muted);
          opacity: 0.7;
        }
      `}</style>
...
/**
 * MorphMany field input — returns null since MorphMany fields don't appear on forms.
 */
