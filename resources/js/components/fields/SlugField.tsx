...
export function SlugFieldInput({
  ...
  const rawReserved = extras.reserved
  // `field` (and therefore `extras.reserved`) can be a freshly-built object
  // on every render — the create/update pages rebuild the field descriptor
  // whenever `dependsOn` overrides are recomputed. Deriving `reserved` as a
  // brand-new array/fallback literal every render (`Array.isArray(...) ? ... : []`)
  // used to give it a fresh identity each time even when the actual word
  // list never changed. That fed straight into the collision-check effect's
  // dependency array below, re-arming the debounced check (and re-rendering
  // via `setCheckState`) on every unrelated re-render — a runaway loop once
  // the parent kept recreating the descriptor. Memoizing on the words'
  // *contents* keeps the reference stable across renders unless the actual
  // reserved list changes.
  const reservedKey = Array.isArray(rawReserved) ? rawReserved.join('\u0000') : ''
  const reserved = useMemo(
    () => (Array.isArray(rawReserved) ? rawReserved : []),
  ...
  // differs from it came from outside; when the form clears the slug that way
  // ("Create & add another" starting the next record) the slug follows its
  // source again. A slug the user empties by hand stays hand-edited. Declared
  // before the auto-generation effect so a cleared slug is seen first.
  ...
  const handleUserInput = (raw: string) => {
    ...
    emit(raw === '' ? null : partialSlugify(raw, separator))
  }

  const handleBlur = () => {
    if (stringValue === '') return
    const canonical = slugify(stringValue, separator)
    if (canonical !== stringValue) emit(canonical === '' ? null : canonical)
  }
