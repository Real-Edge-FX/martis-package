...
/**
 * Built-in page layout: Sidebar Create
 *
 * Alternative to the default full-page create form. Renders a sidebar panel
 * that slides in from the right with the form fields loaded dynamically
 * from the resource schema.
 *
 * Activated by setting overrideCreate() on your Resource:
 *   public function overrideCreate(): ?array
 *   {
 *       return ["component" => "sidebar-create"];
 *   }
 */
export function SidebarCreate({ resourceKey, schema, fields }: CreatePageProps) {
  ...
  const createMutation = useMutation({
    ...
    onError: (err) => {
      if (err instanceof ApiError && err.errors && err.errors.length > 0) {
        ...
        addToast(
          "error",
          err.message || tMsg("validation_errors", "Please fix the errors below."),
        )
      ...
      <div className="flex-1 p-6">
        <nav className="flex items-center gap-2 text-sm mb-6">
          ...
          >
            ...
            <ResourceIcon
              iconName={(schema as unknown as { icon?: string }).icon}
              size={14}
            />
            {schema.label}
          </Link>
        ...
        >
          ...
          >
            {schema.subtitle ?? `${tAct("create")} ${schema.singularLabel}`}
          </p>
          {!sidebarOpen && (
            <button
              type="button"
