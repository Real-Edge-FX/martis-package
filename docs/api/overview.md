...
| `POST` | `/martis/api/resources/{resource}/lenses/{lens}/actions/{action}` | Run a bulk, inline or standalone action of the lens (v2.0.1), as Nova's lens action route. The selected records are the ones the lens's `query()` lists (with the lens filters of `?filters=`); an id it does not list does not resolve (404 when none does). The action's `canSee()` / `canRun()` and the run-action policy apply. |
