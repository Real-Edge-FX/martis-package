...
- When `public/vendor/martis/themes/` is itself a symlink (a shared directory of Deployer or Envoyer, for example), the publish and `martis:theme` never write into its target. They check and back up the files the link shows like any other, warn, and replace the link with a real directory: a full publish removes the link with the wipe, while `--themes-only`, `--no-wipe` and `martis:theme` first copy the target's files (symlinks stay symlinks) into the new directory. The target is left as it was, so drop the directory from the deploy tool's shared list. When the link cannot be replaced, the run stops with exit code 1 and writes nothing.
...
| `public/vendor/martis/themes/` as a symlink (deploy tool shared directory) | Emptied through the link by the wipe | Replaced by a real directory; the target is never written |
