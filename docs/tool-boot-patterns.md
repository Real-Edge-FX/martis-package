...
**Watch out:** the routes register whatever `canSee()` says; the middleware chain is what gates access. `ToolRoutes::middleware($this)` (v1.39.3) is the chain of the Martis API routes (authentication, the 2FA challenge, email verification, the locale, the impersonation expiry, the API throttle) followed by `martis.tool:{uriKey}`, which answers 404 to a user the Tool is hidden from, as its page does. Add a `can:` gate for a finer ability, as `health-check` does above. `ToolRoutes::prefix($this)` is `{martis.path}/api/tools/{uriKey}`, where the SPA's `api` client calls the routes (`ToolRoutes::legacyPrefix($this)`, `martis/api/tools/{uriKey}`, is where `loadRoutes()` has always mounted them). A hand-written list such as `['web', 'martis.auth']` skips the 2FA challenge and the rest, and Martis does not check the routes a tool registers itself: see [Tool routes and their middleware](tools.md#tool-routes-and-their-middleware).
