...
The schema cache clears itself on the upgrade: its key includes the package version, so the forms receive the options in the new order without a manual flush.
...
**The stored-label warning.** Reading a record whose stored value matches the label of a static option and the value of none logs a warning, in production too, once per field per request, naming the model, the record, the field and the value. It fires in both cases above: an array still written label first, and a record saved while it was. Options that come from a closure are not checked.
