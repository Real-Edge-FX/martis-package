...
Martis is a modern, open-source admin engine for Laravel: resources, fields, actions, filters, lenses, metrics and dashboards, with an API compatible with Laravel Nova 5 and a React + TypeScript frontend.
...
| 2 | **[Upgrading](upgrading.md)** | Breaking changes between major versions and what to change in an app: v2.0 reads `Select` and `MultiSelect` options in Nova's order |
...
| 11.6 | **[Keyboard Shortcuts](keyboard-shortcuts.md)** | `addShortcut()` / `disableShortcut()` / `listShortcuts()` registry + `Shift+?` help overlay; mod-key normalisation, two-key sequences, input-focus suppression (v1.1); on `@martis/runtime` for extensions (v1.38.0) |
