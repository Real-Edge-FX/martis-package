...
4. **Recent activity**: the caller's last 5 action events (`martis_action_events` rows where `user_id` is the current user), linking back to the affected record. The block is a read surface of the audit log, so it is gated exactly like the audit-log index: the registered resource for the `ActionEvent` model (the built-in `ActionEventResource`, or a consumer's own) must grant `authorizedToViewAny()` (`static::$policy`, auto-discovery and `Gate::policy()` all apply; with no policy defining `viewAny`, the built-in resource asks the `view-martis-action-events` gate, which denies until you define it, see [Actions → Who can read the audit log](actions.md#who-can-read-the-audit-log-v201)), and when no resource exposes the model at all (`martis.action_events.resource = false` without a replacement) the block is empty for everyone.
