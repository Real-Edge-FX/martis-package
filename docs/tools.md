...
A tool's routes run `ToolRoutes::middleware($tool)` (v2.0; `Martis\Tools\ToolRoutes`): the stack of every protected Martis API route, then the tool's own gate.
...
public function boot(): void
{
    ...
    // `{martis.path}/api/tools/finance-imports`, behind
...
$this->loadRoutes(
    ...
    ?string $prefix = null,                              // defaults to ToolRoutes::prefix($this)
);
```
...
| Tool route gate | The tool's `Authorize` middleware answers `403` to a user the tool is hidden from. | `martis.tool:{uriKey}` in `ToolRoutes::middleware()` answers `404` (v2.0). | The tool's page and metadata already answer `404` to that user, so the app does not reveal which tools it ships. |
