...
index, and each lens row carries `_actionAuthorization` (each action's
`canRun()` on it, as on the resource index) for the lens's actions. The selected records are looked up as the resource index lists
them (its `scopes()` and `indexQuery()`), not through `Lens::query()`: a
lens row the resource index hides cannot be acted on. Before v1.39.3 the
