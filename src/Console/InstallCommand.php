...
class InstallCommand extends Command
{
    ...
    /**
     * The files under `resources/js/martis-extensions/.shims/` (stub name =>
     * path relative to the app root): the modules the consumer's Vite config
     * sends the host's specifiers to, and the TypeScript declarations
     * `tsconfig.extensions.json` maps the same specifiers to. They hold no
     * app code. `martis:install` publishes them with the rest of the
     * scaffold, and the `martis-extension-shims` tag (MartisServiceProvider)
     * rewrites them together after an upgrade.
     *
     * @var array<string, string>
     */
    public const EXTENSION_SHIMS = [
        // v1.9.3+ React shims: the bundle must not emit a bare
        // `import "react"`, which the browser refuses to load ("Failed to
        // resolve module specifier 'react'").
        ...
        // v1.10.0+ runtime shims: `@martis/runtime`, `react-router-dom`,
        // `react-i18next` and `@tanstack/react-query` resolve to the host's
        // copies on `window.Martis.runtime`, so the consumer installs none.
        ...
        // v1.38.0+ `react-dom` shim: the runtime's `createPortal` and, since
        // v1.38.2, `flushSync`, the parts of react-dom an extension can use
        // (the React shim has neither).
        'react-dom-shim.mjs.stub' => 'resources/js/martis-extensions/.shims/react-dom.mjs',
        // v1.38.0+ declarations of the five shims above that `tsconfig.extensions.json`
        // maps (react and react/jsx-runtime come from @types/react), generated by
        // `npm run build:types`.
        'runtime-shim.d.mts.stub' => 'resources/js/martis-extensions/.shims/runtime.d.mts',
        'react-dom-shim.d.mts.stub' => 'resources/js/martis-extensions/.shims/react-dom.d.mts',
