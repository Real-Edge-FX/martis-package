 ...
 * the TTL. The key carries the authenticated user now; guests share one
 * entry, and `$cachePerUser = false` shares one entry for a global metric.
 */
...
/** A metric whose value is the same for everyone, cached once for all users. */
class GlobalCountMetric extends IdentifierAgnosticMetric
{
    protected bool $cachePerUser = false;
}

class GlobalCountMetricWithCacheFor extends GlobalCountMetric
{
    public function cacheFor(): ?DateTimeInterface
    {
        return now()->addMinutes(5);
...
it('shares one entry across users when the metric sets $cachePerUser = false', function (string $metricClass) {
    $metric = $metricClass::make('Count');

    $metric->resolve(metricRequestFor(1));
    $metric->resolve(metricRequestFor(2));
    $metric->resolve(metricRequestFor(null));

    expect(UserScopedCountMetric::$calls)->toBe(1);
})->with([
    'the metrics cache layer' => [GlobalCountMetric::class],
    'a per-class cacheFor()' => [GlobalCountMetricWithCacheFor::class],
]);
