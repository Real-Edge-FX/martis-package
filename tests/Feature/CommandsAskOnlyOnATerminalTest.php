...
it('leaves an existing theme alone without asking when there is no TTY, and asks on one', function (bool $tty, array $asked) {
    // 1.x keeps its exit code: 1 on an existing theme without --force.
    $command = onTerminal(new class extends ThemeMakeCommand
    {
        use FakeTerminal;
    }, $tty, ['name' => 'asked-theme']);

    withExistingFile(resource_path('css/martis/asked-theme.css'), function () use ($command, $asked) {
...
/*
 * A generator whose file exists leaves it alone, through artisan, with the
 * line and the exit code it had on 1.x (a patch release keeps them; v2.0
 * exits 0 everywhere, as Laravel's GeneratorCommand does).
 */
it('leaves an existing generated file alone through artisan, with its 1.x line and exit code', function (string $command, array $arguments, string $relative, ?string $phpClass, string $message, int $exitCode) {
    $created = $phpClass === null ? null : base_path($phpClass);
    $existedBefore = $created !== null && is_file($created);
    try {
        withExistingFile(base_path($relative), function () use ($command, $arguments, $message, $exitCode) {
    ...
    'martis:card' => ['martis:card', ['name' => 'ExitCard'], 'resources/js/martis-extensions/cards/ExitCard.tsx', 'app/Martis/Cards/ExitCard.php', 'already exists', 0],
    'martis:field' => ['martis:field', ['name' => 'ExitField'], 'resources/js/martis-extensions/fields/Exit.tsx', 'app/Martis/Fields/ExitField.php', 'already exists', 0],
    'martis:tool' => ['martis:tool', ['name' => 'ExitTool', '--with-component' => true], 'resources/js/martis-extensions/tools/ExitTool.tsx', 'app/Martis/Tools/ExitTool.php', 'Pass --force to overwrite', 0],
    'martis:component' => ['martis:component', ['name' => 'ExitComponent', '--type' => 'generic'], 'resources/js/martis-extensions/overrides/ExitComponent.tsx', null, 'already exists', 1],
]);
