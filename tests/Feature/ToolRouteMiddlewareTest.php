 ...
 * middleware. Before v1.39.3 the default stack was `['web', 'martis.auth']`:
...
/**
 * Overrides loadRoutes() with its pre-1.39.3 signature and forwards to it,
 * as a tool written before may. Declared here, it also keeps the parent's
...
// ── A signature older tools still load with ─────────────────────────────────

it('loads a tool that overrides loadRoutes() with the pre-1.39.3 signature', function () {
...
it('keeps the list an older override forwards, and warns once naming the tool', function () {
    ...
    expect(toolRouteByUri('martis/api/tools/tool-route-legacy-override/ping')->gatherMiddleware())->toBe(['web', 'martis.auth'])
        ...
        ->toContain("with ['web', 'martis.auth'], the default of Tool::loadRoutes() before v1.39.3")
        ->toContain('the 2FA challenge (martis.2fa)')
        ->toContain('Do not pass a middleware list');
});

it('warns about the pre-1.39.3 list passed explicitly, even with 2FA and email verification off', function () {
...
it('mounts a tool route under martis.path, where the SPA api client calls it', function () {
    ...
    // A patch release keeps the URL 1.x has always served, with the same stack.
    $this->getJson('/martis/api/tools/tool-route-admin-path/ping')->assertOk()->assertExactJson(['pong' => true]);
    expect(toolRouteByUri('admin/api/tools/tool-route-admin-path/ping')->gatherMiddleware())
        ->toBe(toolRouteByUri('martis/api/tools/tool-route-admin-path/ping')->gatherMiddleware());
});

it('keeps the route names on the 1.x URL and names the martis.path copy apart', function () {
    config()->set('martis.path', 'admin');
    file_put_contents($this->routesFile, <<<'PHP'
        <?php
        ...
        // A function declared here would fail if the file were required twice.
        function tool_route_mirror_pong(): array
        {
            return ['pong' => true];
        }

        Route::get('/ping', fn () => response()->json(tool_route_mirror_pong()))->name('tool-route-mirror.ping');
        Route::post('/items/{item}', fn (string $item) => response()->json(['item' => $item]));
        PHP);
    bootToolRoutes(new ToolRouteListTool('tool-route-mirror', $this->routesFile));
    app('router')->getRoutes()->refreshNameLookups();

    expect(route('tool-route-mirror.ping', absolute: false))->toBe('/martis/api/tools/tool-route-mirror/ping')
        ->and(route('martis.tools.tool-route-mirror.path.tool-route-mirror.ping', absolute: false))->toBe('/admin/api/tools/tool-route-mirror/ping');

    $this->actingAs(toolRouteUser(), config('martis.guard'));
    $this->postJson('/admin/api/tools/tool-route-mirror/items/7')->assertOk()->assertExactJson(['item' => '7']);
    $this->postJson('/martis/api/tools/tool-route-mirror/items/7')->assertOk()->assertExactJson(['item' => '7']);
});
