...
- Every docs page opens with a paragraph that describes it, short enough for the docs site to use whole. The site takes a page's description (its search excerpt and meta description) from its first paragraph that is not a blockquote, heading, list, code fence or HTML, and keeps 280 characters of it. That paragraph was a line of navigation on the SSO page (which also said Google, GitHub and Okta ship built in: only Azure does, the others plug in through `MartisSso::extend()`), a table on the preferences page, a partial sentence on the in-app Tool boot page, and ran past 280 characters on the relationships, authorization, cache, generator stubs, impersonation, invitations, repeater and roles pages. The index and the v1 roadmap, which the site does not publish, got the same treatment. `Martis\Support\DocDescription` applies the site's rule, the docs MCP server's `list_docs` now describes a page with it, and `tests/Unit/DocsPageDescriptionTest.php` holds every page to it. +2 Pest.
