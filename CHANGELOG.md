...
- **Relationship counts and one-record cards showed records the related resource hides.** The index count of a `HasMany` / `MorphMany` / `BelongsToMany` / `MorphToMany` field (`showOnIndex()`) and the one-of-many "1 of N" count and `aggregateVia()` tile counted every related record, the ones the related resource's `scopes()` / `indexQuery()` hide included, and a `HasOne` / `MorphOne` card (the one-of-many and Through variants included) showed, edited and deleted its record without the related `view` policy. The counts now apply those hooks (the index aggregates them for the page in its own query, one query instead of one per row), and a card shows and writes its record only when the user may view it (empty card, 404 on Edit / Delete otherwise), as Nova, whose one-record panel is the related detail view and disappears when `view` denies ([nova-dusk-suite HasOneAuthorizationTest](https://github.com/laravel/nova-dusk-suite/blob/10.4/tests/Browser/HasOneAuthorizationTest.php)). **Upgrading from 1.x:** grant `view` where a card should show its record. +11 Pest. See [Upgrading](docs/upgrading.md#relationship-panels-list-what-the-related-index-lists).
