...
- **A relationship write saved any id, whatever its picker offered.** The pickers applied the target's `relatableQuery()`, the source's `relatable{PluralModelName}()` and the field's `relatableQueryUsing()` / `withoutTrashed()`, but the writes did not: `BelongsTo` and `MorphTo` stored any id the request sent, a `Tag` synced any ids, and the `BelongsToMany` / `MorphToMany` attach found the record with a bare `find()`, so a crafted request pointed a record at, or attached, another tenant's record or a soft-deleted one. Every write now checks the id against the query its picker runs, as Nova's `Relatable` and `RelatableAttachment` rules do (Nova 5 source: `Rules/Relatable.php`, `Rules/RelatableAttachment.php`, and the rules `BelongsTo`, `MorphTo`, `BelongsToMany` and `MorphToMany` add in `getRules()` on the field's associatable or attachable query; see also [nova-issues#1440](https://github.com/laravel/nova-issues/issues/1440)): the resource create, update and inline create, the inline create and update of the `HasMany` / `HasOne` / `MorphMany` / `MorphOne` panels, the attach (one record or a batch, which now fails whole before attaching anything) and the pivot update of both many-to-many panels, their pivot fields, and the fields of an Action or pivot action. An id outside the query answers 422 with an error on the field, translated in `en`, `pt_PT` and `pt_BR` (`martis::validation.relatable`, Nova's "This :attribute may not be associated with this resource.", and `relatable_attachment` on the attach's `related_id` / `related_ids`), with the error `code` `relatable`. As in Nova, a soft-deleted record passes only with `{attribute}_trashed=true` in the request (when the related resource's `canViewTrashed()` allows and the field is not `withoutTrashed()`), and a `BelongsTo` / `MorphTo` needs the related record's `add{Model}` policy ability; the write also needs the related resource's `viewAny`, as the picker does, and a `Tag` needs `attachAny{Model}` and `attach{Model}` for the records it adds (Nova's `Tag` field checks neither). The value a record already holds is not checked again, so an update that sends back a target that left the query since keeps saving, and the inverse field of a panel's inline create is not checked (the store writes the parent). The attach modal now sends its picker's `?form[...]` draft with the attach, so a 3-argument `relatableQueryUsing()` closure sees the same values on both. **Upgrading:** widen the hooks to what the app saves; see [Upgrading](docs/upgrading.md#upgrading-to-v201-from-v200). +34 Pest, +1 Vitest.
