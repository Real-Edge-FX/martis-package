...
- **A `KeyValue` edit form showed none of the stored rows.** The input copied its value into local state when it mounted and never read the prop again, but both edit surfaces (the update page and the update drawer) mount the fields before they seed the form from the record, so the rows arrived one render too late: an existing map rendered empty, and adding a row replaced the whole stored map with that single row on save. The input now adopts a value that did not come from its own `onChange` (the record seeding the form, a form reset) and keeps its rows when the form hands back what it just emitted, the same late-value handling `Sparkline` and multiple `BelongsTo` already had. +3 Vitest (component, and the update page rendering the stored rows).
