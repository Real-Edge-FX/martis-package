...
- **Row actions in relationship panels, as in Nova.** A `HasMany`, `HasManyThrough` or `MorphMany` panel now offers the related resource's inline actions (`showInline()`) on each row: the index's row menu, each item disabled where `canRun()` refuses it for that record, run on that record through the same action modal. The panel rows carry the `_actionAuthorization` map index rows carry (one shared `rowActionAuthorizer()` builds both). `BelongsToMany` / `MorphToMany` keep their pivot actions. +2 Pest, +10 Vitest. See [Actions → In relationship panels](docs/actions.md#in-relationship-panels).
