...
- **A `HasManyThrough` panel showed, and acted on, the intermediate record's id.** The `has-many` index paginated the relation's bare query, which on a `hasManyThrough` selects every column of the join, so the intermediate's `id`, timestamps and `deleted_at` overwrote the listed record's: a row showed another record's id, its View / Edit / Delete / Restore / Force delete targeted that record (a Force delete could permanently remove an active record of another parent), and a trashed record came back with `deleted_at: null` when the intermediate soft-deletes. It happened on 1.x for View, Restore and Force delete (fixed there in v1.39.2). The index now paginates through the relation, which selects only the related table's columns, so the panel's sort needs no qualified column (an alias such as a `withCount` column keeps sorting). The `hasManyThrough`, `belongsToMany` and `morphToMany` panels qualify the columns they search by, which the intermediate or the pivot made ambiguous when it has the same column; the other panels and the resource index keep their search columns as written, since their relation or `indexQuery()` may join the table a searchable field reads. +7 Pest.
