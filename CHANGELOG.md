...
10. In each environment, after deploying the flipped arrays: `php artisan martis:publish-assets` (the grouped dropdown and the `''` option fix ship in the frontend bundle), then, last, `php artisan martis:cache:clear schema` (it never expires by default; cleared before the deploy, the next page caches the old options again).
...
- **`withoutOptionOrderWarnings()` on `Select` and `MultiSelect`**, to silence both the plain stored-label warning and the list-shift warning for a field whose data is correct on purpose. A field can legitimately store a valid 0-based position that also reads as another option's label, for example a Nova rating that stores `0..4` and shows `"1".."5"`; without this the warning fires on every request even though nothing is wrong. +3 Pest. See [Fields → Option order](docs/fields.md#option-order).
...
- **`Select::resolve()` now delegates to `Field::resolve()`** instead of reimplementing it, so a future change there (a new seam, a cast) reaches `Select` too; the stored-value check still reads the raw attribute first. `MultiSelect::resolve()` delegates for the `resolveUsing()` branch, where its output matches `Field::resolve()`; the plain-attribute branch stays its own, since it always decodes to a list where `Field::resolve()` returns the raw value. +5 Pest of parity coverage (a plain attribute, `resolveUsing()`, and `computed()`, each compared against a plain `Field`).

- **`Select::resolve()` now delegates to `Field::resolve()`** instead of reimplementing it, so a future change there (a new seam, a cast) reaches `Select` too; the stored-value check reads the raw attribute first, and only when it can warn, so a `computed()` callback runs once per read. `MultiSelect::resolve()` delegates for the `resolveUsing()` branch, where its output matches `Field::resolve()`; the plain-attribute branch stays its own, since it always decodes to a list where `Field::resolve()` returns the raw value. +6 Pest: parity coverage (a plain attribute, `resolveUsing()`, and `computed()`, each compared against a plain `Field`) and a `computed()` callback that runs once.
...
- **A local `vendor/bin/pest` no longer reads a `.env` left in the testbench skeleton.** The suite runs in the testbench skeleton under `vendor/`, and `Orchestra\Testbench\TestCase` loads the skeleton's `.env` when there is one. `vendor/bin/testbench`, which `McpServeCommandTransportTest` spawns, copies the skeleton's `.env.example` to `.env` while it runs and deletes the copy only on a clean exit, so a subprocess killed with SIGKILL, or sent SIGTERM before it registers its signal handlers, left the copy behind. From then on its `CACHE_STORE=database` and `SESSION_DRIVER=cookie` failed about 1050 tests on every local run (`no such table: cache`, `Attempt to read property "cookies" on null`) while CI, which installs a fresh `vendor/`, stayed green on the same commit. PHP 8.5 and the locked Laravel 13.7.0 were not involved: with a fresh `vendor/` both pass. `tests/TestCase.php` sets `$loadEnvironmentVariables = false`, so the tests run on the skeleton's config defaults locally as in CI, and `McpServeCommandTransportTest` removes a `.env` its subprocess leaves. +1 Pest (`TestCaseEnvironmentTest`, which fails on the old `TestCase`). A stale `vendor/orchestra/testbench-core/laravel/.env` can be deleted; the suite ignores it now. The copy also broke `--parallel` from a clean skeleton: while the MCP subprocess ran, the other workers loaded it and failed about 830 tests, whose results exhausted the paratest parent at the 128M CLI memory limit. Parallel runs still hit file races between workers in the shared skeleton (published assets, `config/martis.php`, generated `app/Martis` files); sequential runs do not.
