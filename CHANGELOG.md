...
- **Resource `static $with` for declarative eager loading.** Add `protected static array $with = ['author', 'tags']` to a Resource and Martis applies the `with()` call on every index, relationship-picker, and global-search query. Saves the boilerplate `indexQuery` override that previously had to wrap `$query->with([...])` for the same purpose. Hooked through a new `Resource::applyWith()` method so consumer apps can call it from custom controllers as well.
- **`Resource::redirectAfterCreate()` / `redirectAfterUpdate()` hooks.** Optional per-Resource overrides that customise the post-save destination for the **primary** submit button. Returning a string ships it to the SPA via `meta.redirectTo`; the React `ResourceCreate` and `ResourceUpdate` pages respect it before falling back to the standard "navigate to detail" path. The two extra save variants ("Create & view list" / "Save & continue editing" / etc.) keep their explicit destinations because those encode operator intent.
...
- **Relationships** (`docs/relationships.md` ↔ `core/relationships.mdx`): six fact corrections.
  - `MorphTo::make('commentable_id', ...)` example replaced with `MorphTo::make('commentable', ...)`. The factory takes the relationship method name, never the foreign-key column — MorphTo writes both `_type` and `_id` based on the resolved related record.
  - `canAttach() / canDetach()` example no longer claims a closure signature. Both methods accept a single `bool` (defaults to `true`); for request-aware authorization, override `authorizedToAttach()` / `authorizedToDetach()` on the parent Resource (the docs already showed this fallback further down — the BelongsToMany config example is now consistent).
  - `modalSize()` signature corrected on `BelongsToMany` and `MorphToMany`. Real signature is `modalSize(ModalSize $size, ?string $height = null)`. The previously-undocumented second parameter pins the modal's height to a fixed CSS value (`'70vh'`, `'600px'`).
  - `default_trashed_filter` config key shown in its real location: nested under the `index` block of `config/martis.php`, not at the top level.
  - Internal "Task 10" reference removed from the Hardening section banner.
  - Removed a duplicated `---` separator between MorphTo and HasOne.
- **Fields** (`docs/fields.md` ↔ `core/fields.mdx`): twelve fact corrections against the running code.
  - **`BelongsTo::multiple()` removed.** The doc claimed a many-to-many mode and an associated deferred pivot sync; the method does not exist on `BelongsTo`. Many-to-many is `BelongsToMany`, `MorphToMany`, or `Tag`. The Overrides note no longer claims `fill()` registers a deferred pivot sync.
  - **`modalSize()` signature corrected.** Takes a `ModalSize` enum, never a string. Default is `ModalSize::TwoExtraLarge`. Fixed on both `BelongsTo` and `MorphTo` sections; the inline-create example uses `ModalSize::Large`.
  - **Enum drift fixed on `Currency` and `Code`.** Per the project enum rule, finite-value setters take typed enums: `Currency::currency(CurrencyCode)`, `Currency::displayMode(CurrencyDisplayMode)`, `Code::language(CodeLanguage)`. Examples and getters updated accordingly.
  - **`Slug` gains the missing display methods.** Documented `badgeVariant(string)` (default / muted / accent / success / warning / danger / custom), `badgeAccent()` (sugar for `badgeVariant('accent')`), and `badgeColor(string)` (custom CSS colour, implies `badgeVariant('custom')`).
  - **`BelongsTo` gains `relatableQueryUsing()`.** Per-field constraint on the picker query; complements the resource-level `relatableQuery()`.
  - **New `GuardSelect` section.** Field shipped in v1.8.0 was undocumented. Now lives between `Select` and `MultiSelect` with the canonical `PermissionResource` recipe.
  - **`MorphTo` moved to its logical position** between `MorphMany` and `MorphToMany`. The previous draft placed it at the very end of the file, after `Inline Create`. Single source now.
  - TOC updated for `GuardSelect`.
- **Resources** (`docs/resources.md` ↔ `core/resources.mdx`): "31 field types" corrected to 50. `BelongsTo::make('owner', 'owner', UserResource::class)` cleaned to `'Owner'` (lowercase label was a typo). The internal "Phase 5 of Task 09" leak removed from the Index toolbar resets section. Added documentation for previously-undocumented public API: context-specific field overrides (`fieldsForIndex/Detail/Create/Update/InlineCreate/Preview`), `filters()/lenses()/cards()/dashboards()` on the Resource class, the new `static $with` eager-load list, the per-record `title()` override, `indexSearchable() / searchPlaceholder()` for the search bar, `showMenuCount() / menuCount()` for sidebar count badges, `belongsToSystemSection()`, `searchOrderBy()`, and the new `redirectAfterCreate() / redirectAfterUpdate()` hooks.
- **Troubleshooting** (`docs/troubleshooting.md`, new — was `.mdx`-only): twelve fact corrections against the running code. PHP 8.3+ / Laravel 11/12/13. `--with-profile` no longer claims to create a `Profile` model or admin user; it publishes the avatar + 2FA migrations. Theme tokens live under `config/martis.php` `theme.*` (no separate `config/martis-theme.php`). The login-redirect-loop snippet was a fabricated `martis.auth.guard/provider/driver` block that the package never read; replaced with the real `MARTIS_GUARD` env var and the top-level `guard` config key. 2FA reset now clears all four columns (`two_factor_secret`, `two_factor_recovery_codes`, `two_factor_confirmed_at`, `two_factor_last_used_at`). Resource lookup explained via auto-discovery. `relationshipSearchable()` corrected to `relationSearchable()` and pointed at the relationship fields it actually lives on. `Profile->locale` and `Martis::setLocale()` (neither exist) replaced by the real `PreferencesResolver` chain and `app()->setLocale($locale)`. Metrics no longer claimed to cache by default; the real signature `cacheFor(): ?DateTimeInterface` is shown. Added entries for the v1.8.5 mailer 503 graceful path and the runtime cache-subsystem toggles.

### Changed

- **Internal task IDs scrubbed from the published `config/martis.php`.** Six section banners in the config file leaked development-time labels (`Task 07.1 ⭐ D2`, `D1`, `Task 12`, `Task 15`, `Task 14 ⭐ differential`, `Task 17 ⭐ runtime control`) into a vendor-publish target that consumers see on every install. Replaced each with the plain feature name. No behavioural change.
- **Internal task IDs and Nova mentions scrubbed from `src/`.** Same hygiene pass extended to `src/Resource.php`, `src/Contracts/ResourceContract.php`, `src/MartisServiceProvider.php`, `src/Preferences/PreferencesResolver.php`, `src/Models/UserPreference.php`, `src/Http/Controllers/PreferencesController.php`, `src/Http/Controllers/CacheController.php`, `src/Console/InstallCommand.php`, `src/Enums/AccentColor.php`, `src/Enums/UiDensity.php`, `src/Tools/Tool.php`, `src/Tools/ToolServiceProvider.php`. Internal docblocks no longer reference `Task 07.1 ⭐ D2`, `Task 17`, `Task 12`, `Task 15`, `D1 differential`, `D3` overrides, or "Nova-style" / "Mirrors Nova v5" comparisons. The implementations are unchanged.
...
- Six new Pest cases in `tests/Unit/ResourceWithAndRedirectsTest.php`: two for `applyWith()` (no-op when empty, calls `with()` with the declared list), two for `redirectAfterCreate()` (default null, custom override), two for `redirectAfterUpdate()`.
- Six new Pest cases in `tests/Unit/DefaultRowActionsResolverTest.php` covering the per-action global toggle composition: defaults all on, master enabled false, per-action delete off, AND-composition with a resource whitelist, global wins over a whitelisted action, full opt-out via `defaultRowActions() === false`. Pest 1766 / 1 skipped / 0 failed under `--stop-on-failure`.
