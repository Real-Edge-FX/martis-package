...
- **`HasOneThrough` and `HasManyThrough` work like Nova's: no Create, and their base field's other actions.** A Through panel never offers Create (`canCreate()` has no effect, and stays callable so a resource that calls it keeps loading), as in Nova. Edit and Delete now show by default, as on `HasOne` / `HasMany` (they started hidden), `HasManyThrough` keeps Restore and Force delete on trashed rows, and the `has-one` endpoints take an update or a delete through a `HasOneThrough` field under the related resource's policies (1.x answered 403). An update or a delete through a Through relationship reaches only a record the relationship holds (404 for any other). **Upgrading from 1.x:** clear the schema cache (`php artisan martis:cache:clear schema`), which keeps a panel's 1.x actions until it is rebuilt; to keep the 1.x panel, add `->canUpdate(false)->canDelete(false)` (and `->hideRestoreAction()->hideForceDeleteAction()` on `HasManyThrough`), and deny `update` / `delete` in the related resource's policy to refuse those writes; drop `canCreate()` from Through fields. +21 Pest, +2 Vitest.

- **`HasOneThrough` and `HasManyThrough` work like Nova's: no Create, and their base field's other actions.** A Through panel never offers Create (`canCreate()` has no effect, and stays callable so a resource that calls it keeps loading), as in Nova. Edit and Delete now show by default, as on `HasOne` / `HasMany` (they started hidden), `HasManyThrough` keeps Restore and Force delete on trashed rows, and the `has-one` endpoints take an update or a delete through a `HasOneThrough` field under the related resource's policies (1.x answered 403). An update or a delete through a Through relationship reaches only a record the relationship holds (404 for any other). **Upgrading from 1.x:** to keep the 1.x panel, add `->canUpdate(false)->canDelete(false)` (and `->hideRestoreAction()->hideForceDeleteAction()` on `HasManyThrough`), and deny `update` / `delete` in the related resource's policy to refuse those writes; drop `canCreate()` from Through fields; on a path repository, also clear the schema cache (`php artisan martis:cache:clear schema`). +21 Pest, +2 Vitest.
