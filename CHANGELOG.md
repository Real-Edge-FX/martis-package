...
- **A publish no longer destroys what it cannot write back, and a theme source it cannot read no longer stops it.** Without `--no-wipe`, a published theme without a source (the 1.x case: the theme only existed as the copy the old hint said to edit, often with `martis.theme.name` pointing at it) was deleted with exit 0 and no warning, and `--no-wipe` overwrote a copy edited in place, which 1.x kept. Before it deletes or overwrites a file under `public/vendor/martis/themes/` that it cannot write back (a copy edited in place, a theme without a source, a font next to a theme), `martis:publish-assets` now copies it to `storage/app/martis/theme-backups/<date>-<time>/` and warns with the backup's path and what to do; when a backup cannot be written it stops, exit 1, before deleting anything. A copy it published itself, recorded in `public/vendor/martis/themes/.published.json`, or one identical to its source is replaced without a backup, so the edit-and-publish loop stays quiet. `martis:theme` backs up the published copy the same way before it overwrites it. The publish also warns when `martis.theme.name` names a theme without a source, or a name the panel ignores. A broken symlink or an unreadable file in `resources/css/martis/` stopped the publish after the wipe with a raw `ErrorException`; it is now skipped with a warning, like a `.CSS` extension in uppercase (ignored in silence before). Readability is checked by opening the file, since `is_readable()` can say yes for root on a bind mount that then refuses the read. A published copy that cannot be written fails the run naming the file; a publish record that cannot be written is a warning. `martis:theme:diff` rejects a theme name the panel does not load, warns when the published copy is missing or differs from the source, and no longer says that a publish always deletes a copy without a source (`--no-wipe` keeps it). +35 Pest. See [Theming → Theme files](docs/theming.md#theme-files).
