...
- Six new Pest cases in `tests/Unit/ResourceWithAndRedirectsTest.php`: two for `applyWith()` (no-op when empty, calls `with()` with the declared list), two for `redirectAfterCreate()` (default null, custom override), two for `redirectAfterUpdate()`. Pest 1760 / 1 skipped / 0 failed under `--stop-on-failure`.
