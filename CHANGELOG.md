...
3. Turn lists into maps: a list is keyed 0, 1, 2…, as in Nova, so `['draft', 'published']` now stores `0` and `1`. Pass `array_combine($values, $values)` when each value is its own label, or pass an enum class.
...
6. In a `BulkAssignRole` action generated by `martis:roles` (and in a published `stubs/martis/roles-bulk-assign-role-action.stub`), change `pluck('id', 'name')` to `pluck('name', 'id')`.
7. Look for records saved with the wrong order: reading one logs a warning (static options).

`grep -rn -E "->options\(|optionsFromMap\(" app/` finds the calls; filters are not affected. See [Upgrading](docs/upgrading.md).
