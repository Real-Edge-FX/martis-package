...
- **A tool's routes skipped the 2FA challenge, email verification and the tool's own `canSee()`.** `Tool::loadRoutes()` defaulted to `['web', 'martis.auth']`, while the package's protected API routes also run the impersonation expiry, the configured throttle, `martis.2fa`, `martis.locale` and `martis.verified`: a user who had signed in with a password but not passed the 2FA challenge, or had not verified an email the app requires, called every tool route, and so did a user the tool is hidden from. The default is now `Tool::routeMiddleware()`: the protected API stack, built in one place (`Martis\Http\RouteMiddleware::api()`, which `routes/martis.php` uses too, from `martis.middleware`, `martis.auth_middleware` and `martis.throttle.*`), then `martis.tool:{uriKey}`, which answers 404 to a user the tool is hidden from, as the tool's page does. This follows Nova, whose tool routes run the tool's `Authorize` middleware ([Tools → Routing Authorization](https://nova.laravel.com/docs/v5/customization/tools#routing-authorization)) and, in Laravel's own Pennant tool, Nova's API middleware ([nova-pennant](https://github.com/laravel/nova-pennant/blob/1.x/src/ToolServiceProvider.php)); Nova's gate answers 403. An explicit `$middleware` list is used exactly as given, as before. The docs' inline `Route::middleware(['web', 'martis.auth'])` examples now use `$this->routeMiddleware()`. A `martis.middleware` or `martis.auth_middleware` that is neither a middleware name nor a list of them now throws an `InvalidArgumentException` naming the key (`null` keeps the default). **Upgrading from 1.x:** a tool route now answers 423 before the 2FA challenge, 409 to an unverified user when verification is on, 404 to a user the tool is hidden from, and 429 past the API throttle, which it shares with the rest of the API; a tool that relied on the old stack passes its own list, routes registered in `boot()` with `['web', 'martis.auth']` keep that stack until switched to `$this->routeMiddleware()`, and a subclass that overrides `loadRoutes()` declares `?array $middleware = null`. +15 Pest. See [Tools → Tool routes and their middleware](docs/tools.md#tool-routes-and-their-middleware) and [Upgrading](docs/upgrading.md#tool-routes-run-the-martis-api-middleware).
