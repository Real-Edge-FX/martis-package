...
- **The scaffolded theme's font stacks and PrimeReact bridge drifted from `martis.css`.** The stub's `--martis-font-sans` / `--martis-font-heading` lacked `'Inter var'`, and its `--primary-color-text: #ffffff` pinned the PrimeReact text on accent fills to white, ignoring `--martis-accent-contrast`. They now match `martis.css`.
