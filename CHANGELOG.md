...
- **`HasOneThrough` and `HasManyThrough` work like Nova's: no Create, and their base field's other actions.** A Through panel never offers Create, as in Nova: `canCreate()` has no effect and stays callable so a resource that calls it keeps loading, and `canCreate(true)` raises an `E_USER_DEPRECATED` notice naming the field (Laravel logs it on the deprecations channel) rather than failing silently. Edit and Delete now show by default, as on `HasOne` / `HasMany` (they started hidden), Restore and Force delete on `HasManyThrough` trashed rows do not change (1.x already showed them) and now follow each record's policy like every row action, and the `has-one` endpoints take an update or a delete through a `HasOneThrough` field under the related resource's policies (1.x answered 403). An update or a delete through a Through relationship reaches only a record the relationship holds (404 for any other). **Upgrading from 1.x:** to keep the 1.x panel, add `->canUpdate(false)->canDelete(false)`, and deny `update` / `delete` in the related resource's policy to refuse those writes; drop `canCreate()` from Through fields; on a path repository, also clear the schema cache (`php artisan martis:cache:clear schema`). +21 Pest, +2 Vitest.
...
- **An upgrade kept serving the previous version's cached schema.** The Martis cache keys carried a per-layer counter and no package version, and the `schema` layer has no expiry, so after `composer update` a resource kept serving the fields, flags and actions of the previous version until `martis:cache:clear` or `cache:clear` ran, which neither the upgrade steps nor `martis:install` / `martis:publish-assets` do. Every key now carries the `martis/martis` version Composer installed (`martis:cache:{type}@{installed}:v{N}:…`; a `dev-*` branch, whose name survives `composer update`, also carries its commit reference, `dev-main@0123456789ab`), so an upgrade rebuilds every layer (`metrics`, `navigation`, `dashboards`, `schema` and custom ones). A path repository keeps the version of its last `composer install` / `update`, so there clear the caches after pulling changes. +12 Pest. See [Cache → Invalidation](docs/cache.md#invalidation).
