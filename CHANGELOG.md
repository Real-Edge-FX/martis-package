...
- **A relationship panel returned every row for a negative `per_page`.** The `has-many`, `morph-many`, `belongs-to-many` and `morph-to-many` lists and the two attach searches capped `?per_page=` at 100 but not below it: Laravel ignores a negative limit, so `?per_page=-1` returned every row of the relation (on an attach search, every attachable record) in one response, which any user who could view the parent could repeat to exhaust the server's memory. The page size is now clamped between 1 and 100, as the resource index and the lenses already were. +4 Pest.
- **A `HasManyThrough` panel showed, and acted on, the intermediate record's id.** The `has-many` index paginated the relation's bare query, which on a `hasManyThrough` selects every column of the join, so the intermediate's `id`, timestamps and `deleted_at` overwrote the listed record's: a row showed another record's id, its View / Edit / Delete / Restore / Force delete targeted that record (a Force delete could permanently remove an active record of another parent), and a trashed record came back with `deleted_at: null` when the intermediate soft-deletes. It happened on 1.x for View, Restore and Force delete. The index now paginates through the relation, and the relationship panels qualify the columns they sort and search by, which a joined table with the same column (the intermediate, a pivot) made ambiguous on MySQL and PostgreSQL; the resource index keeps its search columns as written, since its `indexQuery()` may join the table a searchable field reads. +5 Pest.
...
- **`BelongsToMany` and `MorphToMany` ignored `hideSoftDeleteToggle()`, `hideRestoreAction()` and `hideForceDeleteAction()`.** Their panels passed only the search, per-page, create, edit and delete flags to the shared shell, so a trashed related record kept Restore and Force delete, and the trashed filter stayed, whatever the field said. They now honour all three.
