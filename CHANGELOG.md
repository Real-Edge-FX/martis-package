...
- **The Delete of a `HasOneOfMany` / `MorphOneOfMany` card removed another record than the one it showed.** With `latestByTimestamp()` or `oldestByTimestamp()` over a `hasMany` / `morphMany`, the card showed the record that order picks, but `DELETE …/has-one/{relationship}` (and `…/morph-one/{relationship}`), and a `PUT` sent straight to those routes, wrote `$relation->first()`, the first row in storage order: the card's Delete on the newest note deleted the oldest one. The card's Edit opens the related resource's own edit page with the shown id, so it was not affected. Declared over a `hasOneThrough`, the card also showed the intermediate record's id, so its Edit opened, and a write through those routes changed, a record of another parent. Show, update and destroy now read the same record through the relation, a timestamp tie is broken on the primary key (as Eloquent's `latestOfMany()` does), and the related resource's policies run on that record. An Eloquent `latestOfMany()` / `ofMany()` relation was not affected. +18 Pest.
