...
- **An action ran on records the resource's `scopes()` keep out.** The action endpoints (`POST /api/resources/{resource}/actions/{action}` and the single-record `…/{id}/actions/{action}` that the detail page and the inline row actions use) looked the selected ids up through `indexQuery()` only, so with a `scopes()`-based tenancy a user could run an action, queued ones included, on another tenant's records by sending their ids. The records are now looked up through `scopes()`, then `indexQuery()`, as the index lists them. The hooks now run as Eloquent runs a local scope, so an `orWhere()` at their top level is grouped before the selected ids are added: after an `indexQuery()` such as `where('tenant_id', 1)->orWhere('shared', true)`, the lookup read `tenant_id = 1 or (shared and id in (...))`, and an action run on one selected record ran on every record of the tenant; a hook with `and` clauses only runs the same SQL as before. Three more gaps of the same lookup are closed: a run none of whose ids resolved answered 200 after running `handle()` on nothing, and now answers 404 (`One or more selected resources could not be found.`), pivot actions included when the relationship attaches none of the ids; an action that is not `standalone()` posted without `resources` did the same, and now answers 422 (`resources`), as a pivot action already did; and a `standalone()` action received the records the request named, without `canRun()` or the policy being asked, and now always receives an empty collection, as the docs said. **Upgrading:** an action run on a record `scopes()` keeps out is no longer processed (from the detail page of such a record, which still shows it, the run now answers 404); a client that posts a non-standalone action without `resources` must send the ids, or declare the action `standalone()`; and a `standalone()` action that read `$models` must query the records it needs itself. See [Actions → Execution Modes](docs/actions.md#execution-modes) and [Authorization → Declarative query scopes](docs/authorization.md#declarative-query-scopes). +13 Pest.
- **A write through a relationship ignored the related resource's `viewAny`.** `POST`, `PUT` and `DELETE` on the `has-many`, `has-one`, `morph-many` and `morph-one` endpoints checked the parent's `viewAny` / `view` and the related record's `create` / `update` / `delete`, but not the related resource's `viewAny`, which its own per-id endpoints require since v1.34.0 and Nova requires too: a user who could not list `Comment` still wrote comments through a post's panel. Those writes now answer 403 before the related record is queried, and a panel whose related resource denies `viewAny` no longer offers Create, Edit, Delete, Restore or Force delete (it still lists its records). `routable()` is not required, so a headless resource (v1.24.0) keeps working as a relation target. The panel flags live in the cached resource schema, and 1.x schema cache keys carry no package version (the `schema` layer keeps entries with no expiration by default): run `php artisan martis:cache:clear schema` (or `martis:cache:clear`) after upgrading, or the cached panels keep offering those actions, which now answer 403, until the entries are cleared. **Upgrading:** a user whose policy denies `viewAny` on the related resource can no longer create, edit or delete its records through a `HasMany` / `HasOne` / `MorphMany` / `MorphOne` panel or its endpoints; if that user should, grant `viewAny` on the related resource and confine what they see with `indexQuery()`. See [Authorization → `viewAny` is the entry gate](docs/authorization.md#viewany-is-the-entry-gate). +26 Pest.
- **With `MARTIS_GUARD` set to a guard other than the app's default, the panel authorized the wrong user, and the flows around it read the site's users.** `MartisAuthenticate` checked the Martis guard but left the app's default guard in place, so `$request->user()`, `auth()->user()`, the gates and the policies resolved the default guard's user: nobody, or the site user when both guards were signed in in the same browser, whose permissions the panel then applied. The per-user metric key (above) read that user too, so with a custom guard it named every admin "guest" and served the first one's value to the next. The middleware now makes the Martis guard the request's guard, and the rest of the package resolves the Martis guard's users too. The protected routes' throttle ran before that switch (the router's middleware priority put `ThrottleRequests` ahead of `MartisAuthenticate`, which was not in the priority list), so it counted every admin behind one IP in one bucket: `MartisAuthenticate` now implements Laravel's `AuthenticatesRequests`, as Laravel's and Nova's `Authenticate` do, and the throttle counts per Martis user. The magic link looked the email up in `users` and signed that user into the Martis guard, whose next request loaded the admin with the same id (an account takeover, reproduced with a site user and an admin sharing an id; only the admin's 2FA stood in the way): the lookup, the auto-registration and the sign-in now use the Martis guard's provider, and a model without `Notifiable` is mailed at the address. The Martis migrations constrained `martis_user_preferences.user_id` and `invitations.invited_by` / `accepted_user_id` to `users`, and altered `users` for the two-factor and avatar columns: saving an admin's preferences failed on the foreign key (500), or tied the row to the site user with that id, whose deletion then deleted them. They now reference and alter the table of the Martis guard's model; a default install still references `users.id`. The registration pipeline (self-registration and invitation accept) and the profile checked the email's uniqueness in `users`; the registration, the invitation's anti-takeover check and the email verification link read `config('martis.guard', 'web')`, which ignored the app's default guard when `MARTIS_GUARD` is unset. Browser sessions listed (IP, device) and revoked rows by an id that can be another person's (`sessions.user_id` holds the id of whichever guard wrote the row, with no table), and `revoke_sessions_on_demote` signed out the site user who shares the demoted admin's id: when the session guards sign in users of more than one table, the profile now reports the sessions as unsupported and says why, nothing is revoked, and the sweep is skipped with a warning. The audit log, whose user resolves the Martis guard's model, no longer records another guard's user as an actor: the role-change and invitation events of a request of another guard (a site request, even from a browser that also holds a panel session, a job, a command) record no actor or the inviter, an authorization denial is recorded only while the Martis guard is the request's guard, and an impersonation operator signed in by a guard of other users goes to `fields.operator_type` / `operator_id`. `php artisan martis:user` creates the Martis guard's user, as Nova's `nova:user` creates the Nova guard's. **Upgrading:** With a custom `MARTIS_GUARD`, the panel's requests now authenticate as that guard's user everywhere (`MartisAuthenticate` calls `auth()->shouldUse()`, as Laravel's `auth` middleware and Nova's do). Policies, `Gate::before()` / `Gate::after()` callbacks, gates, `canSee()` closures, observers and anything else that reads `auth()->user()` or `auth()->id()` during a panel request receive an instance of that guard's provider model: before, they got the app's default guard's user, which was null, or the site user when both guards were signed in in the same browser (the panel's policies then evaluated the site account). A closure type-hinted on the default model (`fn (User $user)`) now throws a `TypeError`: widen it (`Authenticatable`) or check the instance. The notification bell needs that model to use `Illuminate\Notifications\Notifiable` (without it the bell reads as off and the log says why; or set `MARTIS_NOTIFICATIONS_ENABLED=false`). The action log's and the preferences' user relation resolve that model, and impersonation runs on that guard unless `MARTIS_IMPERSONATION_GUARD` names another. The protected routes' rate limit counts per Martis user rather than per IP. When the guard's model has its own table, repoint the preferences and invitations foreign keys and add the two-factor and avatar columns to that table (the migration is in the upgrade guide), point `MARTIS_AUTH_PASSWORD_BROKER` at a password broker of that provider, and expect the browser sessions section to read as unsupported. For every install, `martis.auth` now runs where Laravel's `auth` runs: right after the session starts, before the throttle, the route bindings and any middleware outside the router's priority list, so a middleware in `martis.middleware` or the `web` group that must run before authentication (one that selects a tenant's database) belongs in that list, as it does for Laravel's `auth`. `MARTIS_IMPERSONATION_GUARD` unset now follows the Martis guard (it defaulted to `web`); a `config/martis.php` published before keeps `'web'` as its default. With the default guard (`MARTIS_GUARD` unset) nothing changes but that order. See [Upgrading → A custom Martis guard](docs/upgrading.md#a-custom-martis-guard) and [`martis.auth` runs where Laravel's `auth` runs](docs/upgrading.md#martisauth-runs-where-laravels-auth-runs). +40 Pest (`MartisGuardSessionTest`, `MartisGuardPanelRequestTest`, `MartisGuardThrottleTest`, `MartisGuardMagicLinkTest`, `MartisGuardAuthFlowsTest`, `MartisGuardMigrationStubsTest`, `MartisGuardBrowserSessionsTest`, which sign in through a real session: `actingAs()` calls `shouldUse()` itself and hid the bugs; `RecordRoleChangeTest`), +2 Vitest.
- **The global search listed, linked and counted the records a resource's `scopes()` hide.** `/api/search` built each resource's query with `indexQuery()` alone, for its results and for the overflow `total`, while the index runs the declarative `scopes()` first and the docs recommend them for multi-tenant confinement: a resource that confined its tenants with `scopes()` sent another tenant's records to the Cmd+K palette (title, subtitle, image and link) and counted them in `total`, on the database and the Scout paths alike. The search now runs `scopes()`, then `indexQuery()`, in the index's order. The lookups that find a parent record through the index's confinement had the same gap and now run both too: the `BelongsToMany` panel (its list, attachable list, attach, detach and pivot update) and the pivot routes (the pivot actions, their fields and pickers, and the pickers of the pivot fields, on `belongs-to-many` and `morph-to-many`) answer 404 for a parent the scopes hide, as for one `indexQuery()` hides. On these surfaces the hooks now run as Eloquent runs a local scope (`Martis\Support\IndexScope`, as the action run above), so an `orWhere()` in them is grouped before the term or the key is added: after `where('tenant_id', 1)->orWhere('shared', true)`, a parent lookup used to resolve the first record of the tenant instead of the one the URL names, and the palette listed the tenant's records whatever the term. A hook with `and` clauses only runs the same SQL as before. The relationship pickers keep `relatableQuery()`, as in Nova ([Relatable Filtering](https://nova.laravel.com/docs/v5/resources/authorization#relatable-filtering)), and the detail, update and delete endpoints keep relying on the policies. **Upgrading:** a scope meant to trim the index page only now also trims the palette and those parent lookups; move it to a filter. +14 Pest. See [Global Search → Which records are searched](docs/global-search.md#which-records-are-searched), [Authorization → Declarative query scopes](docs/authorization.md#declarative-query-scopes) and [Upgrading](docs/upgrading.md#the-global-search-and-the-pivot-routes-apply-scopes).
- **A tool's routes skipped the 2FA challenge, email verification and the tool's own `canSee()`, and ignored `MARTIS_PATH`.** `Tool::loadRoutes()` defaulted to `['web', 'martis.auth']` under `martis/api/tools/{uriKey}`, while the package's protected API routes also run the impersonation expiry, the configured throttle, `martis.2fa`, `martis.locale` and `martis.verified`: a user who had signed in with a password but not passed the 2FA challenge, or had not verified an email the app requires, called every tool route, and so did a user the tool is hidden from; with a `MARTIS_PATH` other than `martis`, the SPA's `api` client, which prefixes `/{MARTIS_PATH}`, did not reach them at all. Called without a middleware list, `loadRoutes()` now runs `ToolRoutes::middleware($tool)` (`Martis\Tools\ToolRoutes`): the protected API stack, built in one place (`Martis\Http\RouteMiddleware::api()`, which `routes/martis.php` uses too, from `martis.middleware`, `martis.auth_middleware` and `martis.throttle.*`, and which is also the new `martis.api` middleware group), then `martis.tool:{uriKey}`, which answers 404 to a user the tool is hidden from, as the tool's page does; the routes stay under `martis/api/tools/{uriKey}` (`ToolRoutes::legacyPrefix($tool)`) with the names the file gives them, and with a `MARTIS_PATH` other than `martis` a copy is also mounted under `ToolRoutes::prefix($tool)`, `{martis.path}/api/tools/{uriKey}`, behind the same middleware, its named routes renamed `martis.tools.{uriKey}.path.{name}` (the file is required once and its routes copied; v2.0 mounts them under `{martis.path}` only). This follows Nova, whose tool routes run the tool's `Authorize` middleware ([Tools → Routing Authorization](https://nova.laravel.com/docs/v5/customization/tools#routing-authorization)) and, in Laravel's own Pennant tool, Nova's API middleware ([nova-pennant](https://github.com/laravel/nova-pennant/blob/1.x/src/ToolServiceProvider.php)); Nova's gate answers 403 (see [Highlights → Tool System](docs/differentials.md#tool-routes-run-behind-the-martis-api-middleware)). The signature keeps its type, `array $middleware = Tool::DEFAULT_ROUTE_MIDDLEWARE`, so an override with the previous signature still loads, and an explicit list is used exactly as given, as before; a list that leaves out the 2FA challenge while `MARTIS_2FA_ENABLED` is on, or email verification while it is on, and the previous default `['web', 'martis.auth']` itself, log a warning naming the tool, once per tool and PHP process. The docs' inline `Route::middleware(['web', 'martis.auth'])->prefix('martis/api/tools/...')` examples now use `ToolRoutes::middleware($this)` and `ToolRoutes::prefix($this)`. A `martis.middleware` or `martis.auth_middleware` that is neither a middleware name nor a list of them, or a throttle limit that is neither a number nor an attribute name, now throws an `InvalidArgumentException` naming the key (`null` keeps the default). **Upgrading:** a tool route now answers 423 before the 2FA challenge, 409 to an unverified user when verification is on, 404 to a user the tool is hidden from, and 429 past the API throttle, which it shares with the rest of the API; its URL does not move (with a custom `MARTIS_PATH` it is also served under `/{MARTIS_PATH}/api/tools/{uriKey}`); a call that passes `['web', 'martis.auth']` keeps that stack and warns, so drop the argument; routes registered in `boot()` with that list keep it, unchecked, until switched to `ToolRoutes::middleware($this)`. +27 Pest. See [Tools → Tool routes and their middleware](docs/tools.md#tool-routes-and-their-middleware) and [Upgrading](docs/upgrading.md#tool-routes-run-the-martis-api-middleware).

### Fixed

- **`martis:publish-assets` deleted the app's custom themes.** Since v1.8.8 the command deleted the whole `public/vendor/martis/` before copying the package's assets, `themes/<name>.css` included: the file `martis:theme` writes there and tells you to edit in place. `martis:install` (the README's upgrade step is `martis:install --force`) and `martis:vendor-publish --assets` run the same publish, so every upgrade took the theme away: its stylesheet answered 404, the panel fell back to the default tokens, and the edits were lost. With `public/vendor/martis` a symlink, the deletion emptied the link's target, and with the package's own `public/` as the target, it deleted the package's compiled assets. The publish now deletes only what the package publishes (`assets/`, `manifest.json`, a Vite `hot` file, and any other top-level entry of the package's `public/`), never `themes/` or anything else you put in `public/vendor/martis/`. A symlink is never followed: a symlinked `public/vendor/martis` is replaced by a real directory holding a copy of its target's other entries (the target keeps its files), a symlinked entry is unlinked, and a destination that is the package's own `public/` is neither deleted nor copied onto. After the copy, each theme source `resources/css/martis/<name>.css` whose published copy is missing is copied to `public/vendor/martis/themes/` (a fresh deploy where `public/vendor/` is not committed got a 404 on the stylesheet); an existing published copy is never overwritten. The `martis:theme` hint now says to keep the edits in `resources/css/martis/` too, and `martis:theme:diff` and the troubleshooting page no longer tell you to run `martis:theme` again to get a missing copy back (with `--force` it overwrites the theme with the scaffold). Flags and exit codes are unchanged. **Upgrading:** nothing to change, but a theme an earlier publish deleted does not come back by itself: restore `public/vendor/martis/themes/<name>.css` from version control or a backup, and keep the edited copy in `resources/css/martis/<name>.css` as well, since that is what a publish restores a missing copy from. A symlinked `public/vendor/martis` becomes a real directory at the next publish. See [Theming → Theme files and asset publishes](docs/theming.md#theme-files-and-asset-publishes). +9 Pest.
- **The Martis commands answered their questions from a pipe.** Every prompt checked Symfony's `isInteractive()` only, which a piped stdin without `--no-interaction` passes, so the pipe answered it (or `docker compose exec -T` waited forever): `yes | php artisan martis:install --with-profile` published a migration adding a `users.y` column and wrote `MARTIS_AVATAR_COLUMN=y`; `yes | php artisan martis:card` (or `martis:field`, `martis:component`, `martis:tool`, `martis:theme`) answered "y" to "already exists. Overwrite?" and rewrote a customised component; `yes | php artisan martis:user` created an admin whose email, name and password were `y`, its email already verified; and the "Run pending migrations now?" of `martis:invitations`, `martis:roles` and `martis:sso`, `martis:sso`'s role mapping and `martis:agents`' Laravel Prompts (which answer their own defaults without a TTY) did the same. They all ask only when the input is interactive **and** stdin is a real TTY now (`Martis\Console\Concerns\AsksOnlyOnATerminal`, the check `martis:install` already used for its feature questions), and otherwise do what they do with `--no-interaction`: the installer takes `profile_picture` as the avatar column (`--existing-avatar-column` needs `--avatar-column` there, and says so), a generator leaves the file alone unless `--force` (with the line and exit code it had: `martis:component` and `martis:theme` exit 1, the others 0), the scaffolds run their migrations, and `martis:agents` wires nothing. `martis:user` without a terminal needs `--email` and `--password`, exits 1 naming the missing one and creates no user; the name defaults to `Martis Admin`. A test fails when a command under `src/Console` asks without the trait. `martis:install --force` also rewrote an application's own `*_create_notifications_table.php` / `*_create_sessions_table.php` (from `make:notifications-table` / `make:session-table`): it now rewrites only a migration whose header holds a sentence of the Martis stub (`Martis in-app notifications table.`, `Martis ships a "Browser sessions" profile section`). **Upgrading:** a script that piped answers into these commands must pass the flags instead (`--with-profile`, `--with-2fa`, `--avatar-column=…`, `martis:user --email=… --password=…`); `yes n | php artisan martis:invitations` (or `martis:roles`, `martis:sso`) used to skip the migrations and now runs them, so pass `--no-migrate`. If you already ran one through a pipe, look for a `users.y` column (a migration adding it under `database/migrations`, `MARTIS_AVATAR_COLUMN=y` in `.env`) and roll it back, for a user whose email is `y` and delete it, and compare a notifications or sessions migration of your own with version control. See [Upgrading → Commands ask only on a terminal](docs/upgrading.md#commands-ask-only-on-a-terminal). +42 Pest.
- **The Composer dist shipped repo-only files.** `.tooling/` (the maintainers' pre-tag script), `phpstan-baseline.neon`, the repository's `AGENTS.md` and a stale `.phpunit.result.cache` were installed into every app's `vendor/martis/martis/`. They are `export-ignore` now, and `.phpunit.result.cache` (already in `.gitignore`) is no longer tracked. +5 Pest.
...
_Not tagged: these fixes first shipped in 1.16.0._
