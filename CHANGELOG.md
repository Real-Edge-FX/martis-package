...
- **Edit and Delete on a `HasOneOfMany` / `MorphOneOfMany` card wrote another record than the one it showed.** With `latestByTimestamp()` or `oldestByTimestamp()` over a `hasMany` / `morphMany`, the card showed the record that order picks, but `PUT` and `DELETE …/has-one/{relationship}` (and `…/morph-one/{relationship}`) wrote `$relation->first()`, the first row in storage order: a card showing the newest note renamed or deleted the oldest one. Show, update and destroy now read the same record. An Eloquent `latestOfMany()` / `ofMany()` relation was not affected. +14 Pest.
