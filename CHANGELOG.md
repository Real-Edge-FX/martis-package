...
- **The tool page header icon and the master row of a tool's settings list were always indigo.** They read `--martis-brand-500`, which no stylesheet or theme defines, so its `#6366f1` fallback won whatever the theme or the `[data-accent]` choice. They now read `--martis-accent`, as the rest of `martis.css` does. Eleven transitions read `--martis-dur-sm`, which is not defined either, through a `120ms` fallback that ignored reduced motion; they now read `--martis-dur-fast` (the same 120ms), which reduced motion clamps to `1ms`.
...
- **The theme has 162 variables, not 94.** The README, the docs index, the differentials, the preferences page and the theming guide said 94 (and the guide "10 logical groups") while `martis.css` defines 160 and the scaffolded theme 162 (the two brand logo heights on top). The theming guide's Variable Reference now lists all 162 in 19 groups with their dark and light values from `martis.css`, the motion section names the five durations that exist, and `tests/Unit/ThemeTokenDriftTest.php` fails when `martis.css` defines a variable the stub or the reference does not list. +3 Pest.
