...
- **The Action Events resource (the `martis_action_events` audit log) is readable only by users granted the new `view-martis-action-events` gate, and its `original` / `changes` no longer reveal values the viewer could not read on the record.** v2.0.0 opened the log, and every value any action changed, to every panel user (a known issue deferred from 2.0.0).
  ...
  - The model's `$hidden` attributes an action changed are stored masked (`[hidden]`, key kept), as Nova stores them (its `ActionEvent` diffs go through `Orchestra\Sidekick\Eloquent\model_state()`, which serialises each `$hidden` attribute as `******`): synchronous and queued actions mask the model's, pivot actions the pivot model's. Rows written before keep their values and are masked on read. New public helper `ActionEventRedactor::maskHiddenAttributes()` for custom writers. +3 Pest.
