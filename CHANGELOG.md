...
- **A one-of-many card over a `hasOneThrough` showed and counted another parent's records.** The card showed the intermediate record's id, so its Edit opened a record of another parent, and its "1 of N" count and `aggregateVia()` tile matched the intermediate table's ids, so they counted and summed another parent's rows. The card now reads the record through the relation, and the count and the tile go through the relation itself, so they also honour the constraints written into it (`->where('paid', true)`) and a custom local key on `hasOne` / `morphOne`.
