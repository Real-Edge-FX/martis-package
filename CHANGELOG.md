...
- **With `MARTIS_GUARD` set to a guard other than the app's default, the panel saw no user.** `MartisAuthenticate` checked the Martis guard but left the app's default guard in place, so `$request->user()` and `auth()->user()` resolved the default guard's user: null. The per-user metric key above named every user "guest", so the first user's value was served to the next (reproduced through the metric endpoint with a session login), and the navigation cache and the policies (`HasPolicy`) saw no user either. The middleware now makes the Martis guard the request's guard, as Laravel's `auth` middleware does (`auth()->shouldUse()`). **Upgrading from 1.x:** with a custom guard, the policies now receive the signed-in user instead of null. +1 Pest (`MartisGuardSessionTest`, which signs in through a real session: `actingAs()` calls `shouldUse()` itself and hid the bug).
