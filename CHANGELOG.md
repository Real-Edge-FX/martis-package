...
Relationship writes now validate the relatable query (Nova's `Relatable` rule): an install whose `relatableQuery()`, `relatable{PluralModelName}()` or `relatableQueryUsing()` is narrower than what it saves gets 422 on those writes. See [Upgrading](docs/upgrading.md#upgrading-to-v201-from-v200).
