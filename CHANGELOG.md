...
- **An action whose selected records all failed to resolve answered "Done", and its row map ignored the policy.** `execute()` looked the ids up through `indexQuery()` with the default scopes, so when none of them resolved (outside the scope, a trashed row, a forged id) `handle()` ran on an empty collection and the run answered 200. It now answers 404 (`One or more selected resources could not be found.`), pivot actions included; when some resolve, the action runs on those, as in Nova. Trashed records are looked up too when the resource soft-deletes, since the index and the panels list them, and a `standalone()` action runs on no record whatever ids are sent. The per-row `_actionAuthorization` map of the index and the panels checked only `canRun()`, so an item stayed enabled when the `runAction` / `runDestructiveAction` policy refused it and the run answered 404; the map and the run now share one predicate. +11 Pest.
