...
- **An action could answer "Done" having handled fewer records than were picked, and its row map ignored the policy.** `execute()` looked the ids up through `indexQuery()` with the default scopes and never checked it found them all, so an id outside the scope, a trashed record or a forged id was dropped and `handle()` ran on what was left (on nothing for a single trashed row) and answered 200. A run now answers 404 (`One or more selected resources could not be found.`) when any id does not resolve, pivot actions included, and looks trashed records up too when the resource soft-deletes, since the index and the panels list them. A `standalone()` action runs on no record whatever ids are sent. The per-row `_actionAuthorization` map of the index and the panels checked only `canRun()`, so an item stayed enabled when the `runAction` / `runDestructiveAction` policy refused it and the run answered 404; the map and the run now share one predicate. **Upgrading from 1.x:** a client that sent ids a user may not reach and relied on the others running gets a 404 now; send only the ids the listing shows. +10 Pest.
