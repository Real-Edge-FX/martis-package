...
- **The edit forms rendered an `immutable()` field as an editable input.** The schema serialises `immutable`, but no form read it: the update page (also when a relation panel opens the record to edit it), the update drawer and the form that edits a pivot row showed an editable input, and the update dropped the new value silently (200, column unchanged). The update forms now render an immutable field read-only, as they render a `readonly()` field, and still submit its stored value: `useMartisForm` marks it `readonly` in `resolvedFields` when its `context` is `update` (so a Tool form bound to a record does the same), and the update drawer and the pivot edit form apply the same `lib/lockImmutableFields.ts`. The create page, the create drawer, the inline create and the attach keep the input editable, and a field inside a `Repeater` row is left alone, since the Repeater writes its rows as one value. `FieldDefinition` types the flag as `immutable?: boolean`. The `Slug` lock hint no longer ends with "after publish", a cause the package cannot know. An input locks only as far as it honours `readonly`: `Avatar`, `BooleanGroup` and `Repeater` ignore it, and `File` / `Image` keep their remove button and drop zone. +14 Vitest. See [Fields → Immutable fields](docs/fields.md#immutable-fields).
