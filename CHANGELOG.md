...
- **The Action Events resource (the `martis_action_events` audit log) is readable only by users granted the new `view-martis-action-events` gate, and its `original` / `changes` no longer reveal values the viewer could not read on the record.** v2.0.0 opened the log, and every value any action changed, to every panel user (a known issue deferred from 2.0.0).
  ...
  - Without access: the index and the detail answer `403`, the sidebar entry and the command palette's *Recent activity* disappear, and a relationship panel listing the log (a `MorphMany` on an `Actionable` model) leaves the detail page, its route answering `403`, as every relationship panel whose related resource denies `viewAny` now does (see Changed).
  - `original` / `changes` keep a value only when the viewer may see that attribute on the record's detail page: a detail field visible to the viewer (`canSee()`, `canSeeForModel()`, or the foreign key / morph type of a visible `BelongsTo` / `MorphTo`), through a resource that lets the viewer `viewAny` and `view` the record. Every other value reads `******`, attributes no field shows (`password`) included; a record the viewer may not view, or one outside the viewer's global scopes, masks every value. Pivot action events mask the pivot model's `$hidden` attributes; events on a model no resource exposes mask the model's `$hidden` attributes. The logic is public as `Martis\Actions\ActionEventRedactor` for custom audit resources.
  ...
  - The model's `$hidden` attributes an action changed are stored masked (`******`, key kept), as Nova stores them (its `ActionEvent` diffs go through `Orchestra\Sidekick\Eloquent\model_state()`, which serialises each `$hidden` attribute as `******`): synchronous and queued actions mask the model's, pivot actions the pivot model's. Rows written before keep their values and are masked on read. New public helper `ActionEventRedactor::maskHiddenAttributes()` for custom writers. +3 Pest.
  - A pivot action's event also masks on read the pivot columns whose pivot field the viewer may not see (`canSee()`) on the `BelongsToMany` / `MorphToMany` field that lists the row, and every column when the viewer sees none of those fields. +2 Pest.
...
- **A crafted `from` link could send the user to another site after a save.** The create and edit pages return to their `from` query parameter (set by a relationship panel's Create and Edit buttons) on Save and Cancel, and passed it to React Router as it came. A value such as `//attacker.example` or `/\attacker.example` is a path to React Router 6 and another origin to the browser, which is the open redirect of [GHSA-wrjc-x8rr-h8h6](https://github.com/advisories/GHSA-wrjc-x8rr-h8h6), fixed only in React Router 7 (the panel now runs on it, see below; the check stays as defence in depth). Martis now follows `from` only when it is a same-origin path (new `safeInternalPath()` helper in `resources/js/lib/safeInternalPath.ts`: it refuses `//`, a backslash, a control character, and anything the URL parser resolves to another origin) and otherwise goes to the default destination. The same check now decides which notification `action_url` and user-menu `customItems` `url` values go through the router: a protocol-relative value was sent to the router as an internal path (the user-menu comment already called it a native link); it now opens as a link, as a full URL does. The other router targets were reviewed and come from route parameters under a fixed `/resources/` prefix, from the resource and config definitions, or from server responses the application writes (`redirectAfterCreate()`, action redirects), never from user or record data. GHSA-337j-9hxr-rhxg (React Router's `deserializeErrors()` in SSR hydration) does not apply: Martis never renders React Router hydration data on the server. See [Relationships](docs/relationships.md). +18 Vitest.
- **The panel runs on React Router 7.18.4, which closes GHSA-wrjc-x8rr-h8h6 and GHSA-337j-9hxr-rhxg; `npm audit --omit=dev` is clean.** The SPA imports `react-router` (library mode, the same `createBrowserRouter` routes) and no longer installs `react-router-dom` or `@remix-run/router`; `RouterProvider` comes from `react-router/dom`, the DOM build that honours `flushSync` navigations. React Router 7 makes the v6 future flags the only behaviour: a navigation's state update runs in `React.startTransition`, and relative paths inside a splat route resolve from the splat. Martis has no relative links inside its one splat route (the 404 page), no loaders, actions or fetchers, and each lazy icon keeps its own `Suspense` boundary, so neither changes what a user sees. React Router 7 needs Node 20 to build (CI already runs Node 20; an app installs the prebuilt assets). Extensions keep working without a rebuild: `window.Martis.runtime.reactRouterDom` is now React Router 7's `react-router-dom` module (every `react-router` export, with the DOM `RouterProvider`), built in the new `resources/js/lib/reactRouterDom.ts`, and every name the `react-router-dom` shim exports exists in it. Names React Router 7 removed (`json`, `defer`, the v6 `UNSAFE_` internals) are gone from the shim's default export. The shim declarations (`react-router-dom.d.mts`) carry the React Router 7 types, every type `react-router` exports; `FutureConfig`, `Hash`, `JsonFunction`, `Pathname`, `Search` and `V7_FormMethod` are gone with it. A scaffold published from this version also sends `react-router` to the `react-router-dom` shim (Vite alias and tsconfig path), so an extension can import from either. See [Upgrading](docs/upgrading.md#the-panel-runs-on-react-router-7) and [Installation](docs/installation-guide.md#extensions-and-react-router-7-v201). The extension contract tests check the served module against `react-router` and type-check a consumer import from `react-router`. +1 Vitest.
- **Dependencies updated for known advisories.** `dompurify` 3.4.16 (GHSA-c2j3-45gr-mqc4, GHSA-55q2-fjhq-7xh7). React Router: see the entry above. The development lock file (`composer.lock`, used by the test suite, not by installing apps) moves to Laravel 13.33, Guzzle 7.15.5, guzzlehttp/psr7 2.13.1, league/commonmark 2.10.3, Symfony 7.4.19, react/http 1.11.1 and phpseclib 3.0.57, which clears the 40 `composer audit` advisories, and its content hash matches `composer.json` again (it predated the v1.16.1 constraint change, so every `composer install` warned that the lock file was out of date). No constraint changed. Laravel 13.33 narrows `__()` to `array|string`, which no longer matched 16 PHPStan baseline entries: the translated lines passed to `string` parameters (the magic-link mail, the default dashboard name, the cache, notification and navigation controllers, the metric ranges) now go through a new internal `Martis\Support\TranslatedLine::get()`, which returns the key when an application overrides a line with an array group, and the baseline was regenerated (153 entries, from 184). +4 Pest.
- **With a custom `MARTIS_GUARD`, password reset read the site's users.** The forgot-password and reset endpoints used the broker `MARTIS_AUTH_PASSWORD_BROKER` named, `users` by default, whatever the guard: beside an `admins` guard, the form looked the email up among the site's users and mailed and reset that account, and an admin could not reset their own password. The broker now comes from the Martis guard (`GuardCatalog::martisPasswordBroker()`): unset, the app's default broker when its provider is the Martis guard's, else the first broker whose provider is (a default install still resolves `users`); set, it must name a broker of `config/auth.php` whose provider is the Martis guard's, or of users of the same table. A broker of another provider, an unknown name, or no fitting broker throws a `Martis\Auth\PasswordBrokerConfigurationException` naming `martis.auth.passwordReset.broker`, which the forgot-password endpoint no longer turns into its "mailer unavailable" 503. The `config/martis.php` default is now `env('MARTIS_AUTH_PASSWORD_BROKER')` (null). Nova reads one broker too (`NOVA_PASSWORDS`, the default broker when null) without checking it against `NOVA_GUARD` ([nova-issues#1989](https://github.com/laravel/nova-issues/issues/1989)). **Upgrading from 2.0.0:** nothing changes with the default guard. With an own guard and password reset on, declare a broker for its provider in `config/auth.php`; a `config/martis.php` published before v2.0.1 still sets `'users'`, which now throws: unset the variable and change the default to `env('MARTIS_AUTH_PASSWORD_BROKER')`, or set it to the guard's broker. +11 Pest (`MartisGuardPasswordBrokerTest`). See [Authentication → Which password broker resets a password](docs/authentication.md#which-password-broker-resets-a-password).
- **The per-request Gate cache keyed a user on its id alone.** `RequestScopedAbilityCache` (`MARTIS_AUTHZ_REQUEST_CACHE=true`) stored `(user id, ability, model)`, so with a custom `MARTIS_GUARD` whose users have their own table the admin 5 and the site user 5 read each other's cached answers in one request (a job or a site request that checks both, for instance). The key now holds the user's morph class and identifier, and `lookup()` takes the user instead of its id. **Upgrading from 2.0.0:** `lookup($user->id, ...)` throws a `TypeError`; pass the user. The package does not call `lookup()` itself. +2 Pest (`AuthorizationFeaturesTest`). See [Authorization → Per-request Gate cache](docs/authorization.md#per-request-gate-cache).
- **The Martis throttles shared Laravel's per-user bucket with the site and with each other.** Laravel's `throttle:{max},{decay}` keys a signed-in user's bucket on `sha1()` of the identifier alone, so with a custom `MARTIS_GUARD` whose users have their own table the admin 5's API requests counted in the bucket of any site route throttled per user for the site user 5 (each could push the other to `429`), and within the panel the 2FA challenge (20 per minute) and the verification email resend (3 per minute) counted in the API's bucket: a resend answered `429` after three API requests in the same minute, and a challenge counted twice. Each Martis throttle now carries a key prefix, the middleware's third parameter, naming its scope and the Martis guard: `martis-api:{guard}:` for the API and the Tool routes (`RouteMiddleware::throttlePrefix()`), `martis-2fa:{guard}:` and `martis-verification:{guard}:`. The public routes keep their per-IP buckets. **Upgrading from 2.0.0:** the API throttle's middleware reads `throttle:120,1,martis-api:web:` (by default); the counters in flight start over once, on deploy. +2 Pest (`MartisGuardThrottleTest`). See [Configuration → API Throttle](docs/configuration.md#api-throttle).
- **A relationship write saved any id, whatever its picker offered.** The pickers applied the target's `relatableQuery()`, the source's `relatable{PluralModelName}()` and the field's `relatableQueryUsing()` / `withoutTrashed()`, but the writes did not: `BelongsTo` and `MorphTo` stored any id the request sent, a `Tag` synced any ids, and the `BelongsToMany` / `MorphToMany` attach found the record with a bare `find()`, so a crafted request pointed a record at, or attached, another tenant's record or a soft-deleted one. Every write now checks the id against the query its picker runs, as Nova's `Relatable` and `RelatableAttachment` rules do (Nova 5 source: `Rules/Relatable.php`, `Rules/RelatableAttachment.php`, and the rules `BelongsTo`, `MorphTo`, `BelongsToMany` and `MorphToMany` add in `getRules()` on the field's associatable or attachable query; see also [nova-issues#1440](https://github.com/laravel/nova-issues/issues/1440)): the resource create, update and inline create, the inline create and update of the `HasMany` / `HasOne` / `MorphMany` / `MorphOne` panels, the attach (one record or a batch, which now fails whole before attaching anything) and the pivot update of both many-to-many panels, their pivot fields, the fields of an Action or pivot action, and the relation fields of a `Repeater`'s rows. An id outside the query answers 422 with an error on the field, translated in `en`, `pt_PT` and `pt_BR` (`martis::validation.relatable`, Nova's "This :attribute may not be associated with this resource.", and `relatable_attachment` on the attach's `related_id` / `related_ids`), with the error `code` `relatable`. As in Nova, the value is checked on every save, the stored one included (a record whose target left the query answers 422 until the target changes); a soft-deleted record passes only with the request's opt-in, `{attribute}_trashed=true` or a value map with `trashed: true`, which a record's `BelongsTo` / `MorphTo` value now carries for a trashed target so the edit form keeps it (when the related resource's `canViewTrashed()` allows and the field is not `withoutTrashed()`); a `BelongsTo` / `MorphTo` needs the related record's `add{Model}` policy ability and fails when the picked record's inverse `HasOne` / `MorphOne` already holds another record (Nova's `relationshipIsFull()`, including its pass for an update that keeps its target or has none yet; `BelongsTo::inverse()` names the inverse, as in Nova). The write also needs the related resource's `viewAny`, as the picker does, and a `Tag` needs `attachAny{Model}` and `attach{Model}` for the records it adds and `detach{Model}` for the ones it removes, a `null` value included (`martis::validation.detachable`; Nova's `Tag` field checks none). The inverse field of a panel's inline create is not checked (the store writes the parent). The attach modals of both `BelongsToMany` and `MorphToMany` now send their picker's `?form[...]` draft with the attach, so a 3-argument `relatableQueryUsing()` closure sees the same values on both. A `BelongsTo` value map the multipart path sends as JSON (an edit form with a file field) is now decoded instead of being written as the foreign key. **Upgrading:** widen the hooks to what the app saves; see [Upgrading](docs/upgrading.md#upgrading-to-v201-from-v200). +41 Pest, +2 Vitest.

### Changed

- **A tool route registered in `boot()` with the v1.x middleware now logs a warning.** v2.0.0 guarded the routes `loadRoutes()` loads with the Martis API stack and warned about a v1.x list passed to it, but a route a tool registered itself with `Route::middleware(['web', 'martis.auth'])->prefix('martis/api/tools/{uriKey}')`, the pattern the v1.x docs showed, kept skipping the 2FA challenge, email verification, the throttle and the tool gate without a word. After the tools boot, `ToolRoutes::warnAboutRegisteredRoutes()` reads the routes under each tool's path (`ToolRoutes::prefix()` and the v1.x `martis/api/tools/{uriKey}`) and logs a warning naming the tool and the route when a route's own middleware is `['web', 'martis.auth']` or leaves out a guard that is on, once per tool and PHP process, and never for a tool already warned about through `loadRoutes()`. The route keeps its middleware. The docs show how to move it to `ToolRoutes::middleware($this)` / `ToolRoutes::prefix($this)` or to a file loaded by `loadRoutes()`. +5 Pest (`ToolRouteMiddlewareTest`). See [Tools → Routes a tool registers in `boot()`](docs/tools.md#routes-a-tool-registers-in-boot).
- **A relationship panel follows its related resource's `viewAny`, as in Nova.** `HasMany`, `HasOne`, `MorphMany`, `MorphOne`, `BelongsToMany`, `MorphToMany` (and `HasManyThrough`, `HasOneThrough`, `HasOneOfMany`, `MorphOneOfMany`) now authorize like Nova's relationship fields (`$resourceClass::authorizedToViewAny($request) && parent::authorize($request)`): a user the related resource does not let `viewAny` does not see the panel on the detail page, the schema or a card, and its routes (list, card, attachable list, attach, detach, pivot update, pivot actions and pickers, writes) answer `403`, as Nova's relationship index does. Before, the panel listed the related records to anyone who could view the parent (v2.0 already refused the writes). A field its own `canSee()` hides keeps answering `404`. New `Martis\Fields\Concerns\AuthorizesRelatedResource`. `ActionEventResource::indexQuery()`, which emptied the log's panel for a viewer without access, is gone: the panel is hidden instead. **Upgrade:** grant `viewAny` on the related resource to users who should keep the panel. See [Upgrading](docs/upgrading.md#relationship-panels-follow-the-related-resources-viewany). +21 Pest.
- **An `Actionable` model's detail page ends with an Action Events panel, as in Nova.** Nova adds a collapsable `MorphMany` of `actions()` through its action resource to the detail fields of a model using `Actionable` (`ResolvesFields::shouldAddActionsField()` / `actionEventsField()`), unless the fields already declare one; Martis now does the same, through the resource that exposes the `ActionEvent` model. The panel follows that resource's `viewAny`, so it stays hidden while the audit log is closed. A resource opts out by overriding `shouldAddActionsField()` (Nova's hook) and changes the panel through `actionEventsField()`. Every detail surface now resolves its fields through the new `Resource::resolveDetailFields()` (`fieldsForDetail()` plus that panel). See [Actions](docs/actions.md#the-action-events-panel-v201). +6 Pest.
- **The action log reads as Nova's.** `ActionEventResource` now mirrors Nova 5's `ActionResource` (fields, order, labels): ID, Name, Initiated By, Target, Status, Original, Changes, Exception, Happened At. Initiated By shows the user's name (else email, else the stored id when the user is gone), resolved through the Martis guard's user model and eager-loaded per page, instead of the raw `user_id`. Target shows the target record's resource label and title, linked to its detail page only when the viewer may `viewAny` and `view` it (Nova's `MorphToActionTarget`); a record gone, or a model no resource exposes, shows the label and id unlinked. Status shows Nova's labels (Waiting for `queued`, Running, Finished for `completed` / `finished`, Failed, plus Denied), with the spinner and the error mark. Original and Changes are key/value tables shown only when the event holds a diff, still redacted by `ActionEventRedactor`; the batch id and the `actionable_*` columns leave the detail page. The resource, its fields, the status labels and the "Action Events" panel of `Actionable` models are translated in `en`, `pt_PT` and `pt_BR` (new `martis::action_events` group). The `fieldsForIndex()` override is gone (the index comes from `fields()`). A `MorphTo` value may now carry `resourceLabel`, which its display prefers over the field's `morphTypes`, and a key/value display renders nested values as JSON instead of `[object Object]`. See [Upgrading](docs/upgrading.md#the-action-log-shows-novas-columns) and [Actions](docs/actions.md#columns-and-detail-fields-v201). +20 Pest (`ActionEventResourceFieldsTest`), +4 Vitest.

### Fixed

- **A relationship panel ignored its fields' `displayUsing()`.** The `HasMany`, `MorphMany`, `BelongsToMany`, `MorphToMany`, `HasOne` and `MorphOne` panels serialized their rows with `resolve()`, while the index and the detail page use `resolveForDisplay()`, so a formatted column (a status label, a name for an id) showed its raw value in a panel only: the Action Events panel of a record read `1` and `completed` where the log's index read the user's name and `Finished`. The panels now resolve for display, as Nova's index fields do. +1 Pest.
- **The action log offered Edit and Delete controls on each row.** They were disabled, since an event is never updated or deleted, but Nova's action log shows only View: `ActionEventResource::defaultRowActions()` returns `[View]`. +1 Pest.
- **A queued action run by the `sync` queue connection (or a worker that picks it up at once) left its events at `queued` with no diff.** The `queued` events were written after the job was dispatched, so a job that ran immediately found none to settle. They are now written before the dispatch, as a pivot action's already were.
- **Most of the borders the components draw with Tailwind utilities were missing.** The same cause as the loading spinners fixed in 2.0.0: 79 elements in 29 files set a border width (`border`, `border-2`, `border-b`, `border-t`, `divide-y`) with no border style, so the browser drew nothing (a native input kept its own inset border). They now draw a solid 1px border in `--martis-border` (the accent or danger colour where the element sets one): the action, table-row, lens, icon-picker and repeater "Add row" menus, which now match the PrimeReact menus, and the action, lens and icon-picker triggers; the create and update form cards and their footer, the attach dialogs' search bar and pivot fields, the inline-create dialog's rows, the lens summary row; the profile page's cards, session rows, avatar ring and photo buttons, the two-factor QR and recovery-code boxes; the relation peek cards, the create-related and timezone buttons, the file rows, image thumbnails and repeater rows; the preferences colour swatch, hex input, language select and preset note; the schema and custom-layout error boxes; the component inspector's panels; the input `martis:component --type=field` generates (a copy of `component-field.tsx.stub` published to `stubs/martis/` keeps the old class, and components generated before keep it too: add `border-solid` next to `border`). An element sized by its padding is 2px taller (1px for a one-sided separator), as Tailwind draws these utilities. A one-sided border now zeroes the other sides (`border-0 border-b border-solid`): a style alone draws the browser's 3px default width on every side no utility sets. Five width classes that were never drawn are gone instead: the topbar preferences button stays borderless like the other 34px topbar buttons (drawn, its transparent border made it 36px), the create and update loading skeletons lose their row separators (the form they stand for has none, and the last one doubled the card's bottom border), and the `BelongsTo` id input and the `MorphTo` type select drop a `border` their `.martis-input` class already draws. The error boundary's fallback card used utilities no stylesheet defines (`border-destructive/30`, `bg-card`, `text-muted-foreground`, `bg-primary`), so it showed unstyled text and a bare "Try again": it now reads the danger, card and text tokens and uses `martis-btn-primary`. The update page's loading skeleton border read Tailwind's fixed grays and now reads `--martis-border`, and the icon picker's search input loses the browser's inset border. No stylesheet rule changes: the five `martis.css` rules that set only a border width (the preferences and notification arrows, the tooltip arrow) take their style from another rule for the same element.
- **The `sessions` and `notifications` migrations shaped a shared table on the Martis guard's users alone.** Every session guard writes `sessions.user_id` and the site's notifications land in `notifications`, but v2.0.0 took the key type of the Martis guard's model only: beside an `admins` guard keyed by UUID, the site's bigint ids no longer fit the column (PostgreSQL and MySQL reject them on every request that writes the session), or the other way round. The two stubs now read the models of the Martis guard, the default guard and every `session` guard, and create a string column (string `notifiable_*` morphs) when their keys differ. A string-keyed model got bigint `morphs()` on `notifications`; it now gets string morphs. Both migrations still skip an existing table, as Laravel 11+ creates `sessions`: the installation guide gives the symptom and the one-off migration that widens it. `MARTIS_USER_ID_COLUMN_TYPE` still overrides. +8 Pest (`MartisGuardMigrationStubsTest`). See [Installation → The shared `sessions` and `notifications` tables](docs/installation-guide.md#the-shared-sessions-and-notifications-tables).
- **An `orWhere()` in a user hook no longer escapes the filters, the search or a fence.** The index composed the resource's `scopes()` and `indexQuery()`, its filters and its search raw, so a hook such as `where('tenant_id', 1)->orWhere('shared', true)` read `tenant_id = 1 or (shared and <filters> and <search>)`: every record of the tenant showed whatever the filters and the search said, and a filter or a `searchQuery()` written with `orWhere()` listed another tenant's records. Every user hook now runs as Eloquent runs a local scope (`IndexScope::grouped()`, the one implementation `RelationScope` now shares): the index hooks on the index page and its count badge, each index filter, `searchQuery()`, each filter a lens's `withFilters()` applies, and the three picker layers (`relatableQuery()`, `relatable{PluralModelName}()`, `relatableQueryUsing()`, on the BelongsTo, context-free, attachable BelongsToMany and MorphToMany pickers). A hook that starts with `orWhere()` reads as `and` there, so `?trashed=only` no longer lists active records under it. Hooks that add only `and` clauses produce the same SQL. See [Upgrading to v2.0.1](docs/upgrading.md#upgrading-to-v201-from-v200). +12 Pest.
- **An action only a lens declares runs from the lens.** The lens page ran its actions through the resource's action routes, which look up the resource's `actions()` only, so an action declared in the lens's own `actions()` answered 404 (its fields, its pickers and its run), and a resource action the lens left out could still run. The lens page now uses the new lens action routes, `/martis/api/resources/{resource}/lenses/{lens}/actions` (list), `.../{action}/fields`, `.../{action}/relatable/{field}` and `POST .../{action}`, which resolve the action from the lens (its own `actions()`, or the resource's when it does not override it), as Nova's lens action routes do, and are gated by the lens's `canSee()`. The selected records are the ones the lens lists, as Nova's `LensActionRequest` reads them: `Lens::query()` on the resource's model with the lens's filters, the ids it does not list answering 404, and whole records read by key so a lens that selects aggregates still hands its actions real models (a lens whose `query()` returns a paginator throws a `LogicException` on an action run). Lens rows now carry `_actionAuthorization` for the lens's actions, so an inline action disabled by `canRun()` shows disabled. Rebuild and republish the assets (`php artisan martis:publish-assets`). +15 Pest, +1 Vitest.
- **A resource index the user may not list showed the 403 page and an "Error / Records could not be loaded" toast.** The index page fetches the schema and the records in parallel; when both answered 403 (a resource the user may not `viewAny`, such as the closed action log) the page rendered the error page and the records' failure toasted on top of it. The toast now waits for the schema to settle and stays quiet when the page is the error page (403, 404 or any schema failure); a records failure under a working schema still toasts once, even when it arrives before the schema. +3 Vitest (`ResourceIndex.fetch-error.test.tsx`).
- **A batch attach attached a record a second time.** The duplicate check of `related_ids` on a `BelongsToMany` / `MorphToMany` attach narrowed the relationship itself instead of a copy, so after the first record every check matched nothing and an already attached record was attached again (the single attach narrowed it the same way before its pivot data was read). It now checks on a copy. Covered by `RelatableWritesTest`.

### Docs

- **Borders.** The theming guide explains why the bundled CSS has no Tailwind preflight and how to write a border without it (`border border-solid`, `border-0 border-b border-solid`, `divide-y divide-x-0 divide-solid`, a theme token for the colour, a style word in an inline shorthand). The helper-class example and the override input example carried a border utility with no style; they now carry `border-solid`.

### Internal

- **Every border in the components is checked, not only the spinners.** `resources/js/themeTokenReads.test.ts` reads each class attribute through its literals, templates, concatenations (`+`, and `+=` on a variable, whose appended pieces count as conditional), conditions, `[...].join(' ')`, the class helpers (`clsx`, `cn`, `classNames`, `twMerge`: arguments, nested arrays and object keys, a key applied always only when its value is `true`) and the constants and lookup tables of its file, with the element's inline `style`, in the components and the React stubs: a width utility needs a style utility in the same string or in one that is always applied, a style needs a width on every side, a divide width needs a divide style and both axes, and an inline border shorthand needs a style word. A class string it cannot follow to an element (a prop, another module) is checked where it is written. The same walk now checks the variable names written whole (`getPropertyValue`, `setProperty`, `removeProperty`, `cssVar()`, an inline custom property) against the defined variables, and `ALLOWED_UNDEFINED` against the "Not counted" paragraph of the theming guide, as the Pest list is. `tests/Unit/BorderStyleCssTest.php` fails on a border shorthand in `martis.css` without a style. `ThemeTokenDriftTest` names a rule without the spacing CSS ignores, so a theme stub that writes `@media(prefers-reduced-motion:reduce)` is compared with the bundled `@media (prefers-reduced-motion: reduce)` block instead of failing as a missing one, while `html :not(.dark)` (a descendant) stays apart from `html:not(.dark)`. Each route has a refusal probe and its nearest accepted case, and each check was seen to fail on its mutant. +4 Vitest (7 tests where the spinner walk had 3), +3 Pest.
...
- **Relationship counts and one-record cards showed records the related resource hides.** The index count of a `HasMany` / `MorphMany` / `BelongsToMany` / `MorphToMany` field (`showOnIndex()`) and the one-of-many "1 of N" count and `aggregateVia()` tile counted every related record, the ones the related resource's `scopes()` / `indexQuery()` hide included, and a `HasOne` / `MorphOne` card (the one-of-many and Through variants included) showed, edited and deleted its record without the related `view` policy. The counts now apply those hooks (the index aggregates them for the page in its own query, one query instead of one per row), and a card shows and writes its record only when the user may view it, as Nova, whose one-record panel is the related detail view and disappears when `view` denies ([nova-dusk-suite HasOneAuthorizationTest](https://github.com/laravel/nova-dusk-suite/blob/10.4/tests/Browser/HasOneAuthorizationTest.php)): otherwise the endpoint answers `meta.hidden`, the card is not rendered at all (no Create, which would add a second record, no Edit, no count), and Edit / Delete answer 404. A second record on a `HasOne` / `MorphOne` card answers 422 (`The HasOne relationship has already been filled.`, Nova's wording for a `HasOne`; the `MorphOne` sentence is Martis's) instead of 500, with the sentence as the response's `message` too (the form has no input for the relationship, so a field error alone never showed), whether or not the user may view the one there, and the check runs again under a lock on the parent before the insert, so two concurrent creates cannot both pass it. Only that check and the insert run in the lock's transaction: the related resource's `afterSave()` and the deferred writes of the form (a `Tag`, a `Repeater`) run after the commit, as on every other create, so a job they dispatch finds the record; a one-of-many card takes more records and its create opens no transaction. Unlike Nova, which reads a `HasOne` without global scopes and so shows a soft-deleted record (nova-issues discussion #4120), a card leaves a trashed record out. **Upgrading from 1.x:** grant `view` where a card should show its record; a custom card component should render nothing on `meta.hidden`. +21 Pest, +10 Vitest. See [Upgrading](docs/upgrading.md#relationship-panels-list-what-the-related-index-lists).
- **A relationship panel listed the rows its related resource's index hides.** The `has-many` (`HasMany`, `HasManyThrough`), `morph-many`, `belongs-to-many` and `morph-to-many` indexes paginated the relationship without the related resource's `scopes()` and `indexQuery()`, so a resource that scoped its tenancy or visibility with them showed the hidden rows (and counted them) on another resource's detail page. They now apply both, in the index's order, as Nova's relationship index runs the related resource's `indexQuery()` ([nova-issues#971](https://github.com/laravel/nova-issues/issues/971), [#3655](https://github.com/laravel/nova-issues/issues/3655), [#337](https://github.com/laravel/nova-issues/issues/337)), without letting a hook written for the index break the panel or reach another parent's rows: on a plain `HasMany` / `MorphMany` panel they run as a grouped Eloquent scope, so an `orWhere()` stays inside the group (Nova issue #3655); on a Through or pivot panel and for every count they run on a fresh query of the related model and the rows are kept by key, so a `select()`, a join to the parent, the pivot or the intermediate table, or an unqualified column works as on the index instead of answering 500, and an index count stays per parent. The *Only trashed* filter keeps working with a hook that returns its own query. The inline create buttons (`BelongsTo`, `MorphTo`, `Tag`, `BelongsToMany`, `MorphToMany`) now need the related `viewAny` as well as `create`, and the create form's `sync-field` / field-options endpoints need `viewAny`, like the create endpoints. A hook's leading `orWhere()` stays inside its group, a relation defined `withTrashed()` keeps its trashed records in its counts and panels (the outer query decides the trashed state, never the key subquery), and so does a relation that removes another global scope (`->withoutGlobalScope(ArchivedScope::class)`, `->withoutGlobalScopes()`): the key subquery and the query the hooks receive leave out the scopes the relation removes, and so does the many relationship an Eloquent one-of-many card (`latestOfMany()`) rebuilds for its "1 of N" count. A hook with a `having` on a `withCount` alias works by key (MySQL), and the second-record lock runs on the parent model's own connection. The index counts resolve the relationship as `withCount()` does, on a model that holds no record: a relationship method that throws there (it reads an attribute of a loaded record) is counted per row instead of failing the index, and one that reads the parent's attributes without throwing counts the wrong rows (see Upgrading). An action run from a panel finds its records as that panel lists them: a run from a `HasMany`, `HasManyThrough` or `MorphMany` panel (`viaResource`, `viaResourceId`, `viaRelationship`) read them through the resource's own index, which put back a global scope the relationship removes, so an archived row the panel showed answered 404; it now reads the relationship's rows, narrowed by the resource's `scopes()` and `indexQuery()` by key, with the trashed ones only when the panel offers its trashed filter. A pivot action (`BelongsToMany`, `MorphToMany`) ignored the related resource's hooks and ran on a row the panel hides; it now answers 404 there, and reaches a trashed row the panel's trashed filter lists. A `standalone()` action checks the relationship its run names, and a queued action's job reloads the records the run resolved by key without global scopes (`ExecuteAction`, `ExecutePivotAction`), as Laravel restores a job's models, so a trashed record the run resolved is no longer left out. **Upgrading from 1.x:** on Through and pivot panels the hooks' order and aliases do not reach the rows, and a hook's order comes before a plain panel's `?sort=` (as on the index); see Upgrading. +115 Pest (checked on SQLite, MySQL 8 and PostgreSQL 16 with `MARTIS_TEST_DB`; the concurrent-create cases, for a `HasOne` and a `MorphOne` card, need MySQL or PostgreSQL). See [Upgrading](docs/upgrading.md#relationship-panels-list-what-the-related-index-lists).
- **A record could be created without `viewAny`.** `POST /api/resources/{resource}` and the inline create form and store checked `create` only, while the show, update and destroy endpoints require `viewAny` first since v1.34.0 (Nova 2 refused every resource route that way, [nova-issues#1762](https://github.com/laravel/nova-issues/issues/1762); no public source covers Nova 4 or 5). They now answer 403 without it. **Upgrading from 1.x:** grant `viewAny` to users who create records. +3 Pest.
...
- **A metric's cached result was served to every user.** Both cache paths of `Metric::resolve()` (the `metrics` layer and a per-class `cacheFor()`) keyed the entry on the metric, range, filters and locale only, so a `calculate()` scoped to the user, their tenant or their permissions served the first user's value to everyone for the TTL (5 minutes by default). The key now carries the authenticated user (`Metric::resultCacheKey()`); guests share one entry, and a user whose identifier is not an int, a string or `Stringable` (a UUID or ULID value object is `Stringable`) gets no cached entry rather than the guests' one. Laravel Nova keys its metric cache without the user; Martis does not follow it here because the leak is a data exposure. Effect on an existing installation: no previous metric entry is reused (every key changes, so each user's first request computes the metric again and the old entries expire by their TTL), and a metric that is the same for everyone is now computed and stored once per user, so the cache holds more entries and hits less often; raise `MARTIS_CACHE_METRICS_TTL` or the `cacheFor()` lifetime for heavy global metrics, or set `protected bool $cachePerUser = false;` on a metric whose value does not depend on who asks. See [Cache](docs/cache.md#the-four-built-in-layers). +12 Pest (`MetricCacheUserScopeTest`: the "each user gets their own value" and identifier cases fail on the previous key, a binary identifier and a malformed filter included).
- **With `MARTIS_GUARD` set to a guard other than the app's default, the panel authorized the wrong user, and the flows around it read the site's users.** `MartisAuthenticate` checked the Martis guard but left the app's default guard in place, so `$request->user()`, `auth()->user()`, the gates and the policies resolved the default guard's user: nobody, or the site user when both guards were signed in in the same browser, whose permissions the panel then applied. The per-user metric key named every user "guest", so the first user's value was served to the next (reproduced through the metric endpoint with a session login). The middleware now makes the Martis guard the request's guard. The metric key names the user by model and identifier in serialized segments (a filter string could collide with a user id before), and `$cachePerUser = false` shares a global metric's entry. The rest of the package now resolves the Martis guard's users too. The protected routes' throttle ran before that switch (the router's middleware priority put `ThrottleRequests` ahead of `MartisAuthenticate`, which was not in the priority list), so it counted every admin behind one IP in one bucket: `MartisAuthenticate` now implements Laravel's `AuthenticatesRequests`, as Laravel's and Nova's `Authenticate` do, and the throttle counts per Martis user. The magic link looked the email up in `users` and signed that user into the Martis guard, whose next request loaded the admin with the same id (an account takeover, reproduced with a site user and an admin sharing an id; only the admin's 2FA stood in the way): the lookup, the auto-registration and the sign-in now use the Martis guard's provider, and a model without `Notifiable` is mailed at the address. The Martis migrations constrained `martis_user_preferences.user_id` and `invitations.invited_by` / `accepted_user_id` to `users`, and altered `users` for the two-factor and avatar columns: saving an admin's preferences failed on the foreign key (500), or tied the row to the site user with that id, whose deletion then deleted them. They now reference and alter the table of the Martis guard's model; a default install still references `users.id`. The registration pipeline (self-registration and invitation accept) and the profile checked the email's uniqueness in `users`; the registration, the invitation's anti-takeover check and the email verification link read `config('martis.guard', 'web')`, which ignored the app's default guard when `MARTIS_GUARD` is unset. Browser sessions listed (IP, device) and revoked rows by an id that can be another person's (`sessions.user_id` holds the id of whichever guard wrote the row, with no table), and `revoke_sessions_on_demote` signed out the site user who shares the demoted admin's id: when the session guards sign in users of more than one table, the profile now reports the sessions as unsupported and says why, nothing is revoked, and the sweep is skipped with a warning. The audit log, whose user resolves the Martis guard's model, no longer records another guard's user as an actor: the role-change and invitation events of a request of another guard (a site request, even from a browser that also holds a panel session, a job, a command) record no actor or the inviter, an authorization denial is recorded only while the Martis guard is the request's guard, and an impersonation operator signed in by a guard of other users goes to `fields.operator_type` / `operator_id`. `php artisan martis:user` creates the Martis guard's user, as Nova's `nova:user` creates the Nova guard's. **Upgrading from 1.x:** With a custom `MARTIS_GUARD`, the panel's requests now authenticate as that guard's user everywhere (`MartisAuthenticate` calls `auth()->shouldUse()`, as Laravel's `auth` middleware and Nova's do). Policies, `Gate::before()` / `Gate::after()` callbacks, gates, `canSee()` closures, observers and anything else that reads `auth()->user()` or `auth()->id()` during a panel request receive an instance of that guard's provider model: before, they got the app's default guard's user, which was null, or the site user when both guards were signed in in the same browser (the panel's policies then evaluated the site account). A closure type-hinted on the default model (`fn (User $user)`) now throws a `TypeError`: widen it (`Authenticatable`) or check the instance. The notification bell needs that model to use `Illuminate\Notifications\Notifiable` (without it the bell reads as off and the log says why; or set `MARTIS_NOTIFICATIONS_ENABLED=false`). The action log's and the preferences' user relation resolve that model, and impersonation runs on that guard unless `MARTIS_IMPERSONATION_GUARD` names another. The protected routes' rate limit counts per Martis user rather than per IP. When the guard's model has its own table, repoint the preferences and invitations foreign keys and add the two-factor and avatar columns to that table (the migration is in the upgrade guide), point `MARTIS_AUTH_PASSWORD_BROKER` at a password broker of that provider, and expect the browser sessions section to read as unsupported. For every install, `martis.auth` now runs where Laravel's `auth` runs: right after the session starts, before the throttle, the route bindings and any middleware outside the router's priority list, so a middleware in `martis.middleware` or the `web` group that must run before authentication (one that selects a tenant's database) belongs in that list, as it does for Laravel's `auth`. See [Upgrading → A custom Martis guard](docs/upgrading.md#a-custom-martis-guard). +40 Pest (`MartisGuardSessionTest`, `MartisGuardPanelRequestTest`, `MartisGuardThrottleTest`, `MartisGuardMagicLinkTest`, `MartisGuardAuthFlowsTest`, `MartisGuardMigrationStubsTest`, `MartisGuardBrowserSessionsTest`, which sign in through a real session: `actingAs()` calls `shouldUse()` itself and hid the bugs; `RecordRoleChangeTest`), +2 Vitest.
...
- **The global search listed, linked and counted the records a resource's `scopes()` hide.** `/api/search` built each resource's query with `indexQuery()` alone, for its results and for the overflow `total`, while the index runs the declarative `scopes()` first and the docs recommend them for multi-tenant confinement: a resource that confined its tenants with `scopes()` sent another tenant's records to the Cmd+K palette (title, subtitle, image and link) and counted them in `total`, on the database and the Scout paths alike. The search now runs `scopes()`, then `indexQuery()`, in the index's order. The lookups that find a parent record through the index's confinement had the same gap and now run both too: the `BelongsToMany` panel (its list, attachable list, attach, detach and pivot update) and the pivot routes (the pivot actions, their fields and pickers, and the pickers of the pivot fields, on `belongs-to-many` and `morph-to-many`) answer 404 for a parent the scopes hide, as for one `indexQuery()` hides. On these surfaces and on an action run, the hooks now run as Eloquent runs a local scope, so an `orWhere()` in them is grouped before the term, the key or the selected ids are added: after `where('tenant_id', 1)->orWhere('shared', true)`, a parent lookup used to resolve the first record of the tenant instead of the one the URL names, an action run on one selected record ran on every record of the tenant, and the palette listed the tenant's records whatever the term. A hook with `and` clauses only runs the same SQL as before. The relationship pickers keep `relatableQuery()`, as in Nova ([Relatable Filtering](https://nova.laravel.com/docs/v5/resources/authorization#relatable-filtering)), and the detail, update and delete endpoints keep relying on the policies. **Upgrading from 1.x:** a scope meant to trim the index page only now also trims the palette and those parent lookups; move it to a filter. +14 Pest. See [Global Search → Which records are searched](docs/global-search.md#which-records-are-searched), [Authorization → Declarative query scopes](docs/authorization.md#declarative-query-scopes) and [Upgrading](docs/upgrading.md#the-global-search-and-the-pivot-routes-apply-scopes).
- **A tool's routes skipped the 2FA challenge, email verification and the tool's own `canSee()`, and ignored `MARTIS_PATH`.** `Tool::loadRoutes()` defaulted to `['web', 'martis.auth']` under `martis/api/tools/{uriKey}`, while the package's protected API routes also run the impersonation expiry, the configured throttle, `martis.2fa`, `martis.locale` and `martis.verified`: a user who had signed in with a password but not passed the 2FA challenge, or had not verified an email the app requires, called every tool route, and so did a user the tool is hidden from; with a `MARTIS_PATH` other than `martis`, the SPA's `api` client, which prefixes `/{MARTIS_PATH}`, did not reach them at all. Called without a middleware list, `loadRoutes()` now runs `ToolRoutes::middleware($tool)` (`Martis\Tools\ToolRoutes`): the protected API stack, built in one place (`Martis\Http\RouteMiddleware::api()`, which `routes/martis.php` uses too, from `martis.middleware`, `martis.auth_middleware` and `martis.throttle.*`, and which is also the new `martis.api` middleware group), then `martis.tool:{uriKey}`, which answers 404 to a user the tool is hidden from, as the tool's page does; its prefix is `ToolRoutes::prefix($tool)`, `{martis.path}/api/tools/{uriKey}`. This follows Nova, whose tool routes run the tool's `Authorize` middleware ([Tools → Routing Authorization](https://nova.laravel.com/docs/v5/customization/tools#routing-authorization)) and, in Laravel's own Pennant tool, Nova's API middleware ([nova-pennant](https://github.com/laravel/nova-pennant/blob/1.x/src/ToolServiceProvider.php)); Nova's gate answers 403 (see [Highlights → Tool System](docs/differentials.md#tool-routes-run-behind-the-martis-api-middleware)). The signature keeps its v1.x type, `array $middleware = Tool::DEFAULT_ROUTE_MIDDLEWARE`, so an override with the v1.x signature still loads, and an explicit list is used exactly as given, as before; a list that leaves out the 2FA challenge while `MARTIS_2FA_ENABLED` is on, or email verification while it is on, and the v1.x list `['web', 'martis.auth']` itself, log a warning naming the tool, once per tool and PHP process. The docs' inline `Route::middleware(['web', 'martis.auth'])->prefix('martis/api/tools/...')` examples now use `ToolRoutes::middleware($this)` and `ToolRoutes::prefix($this)`. A `martis.middleware` or `martis.auth_middleware` that is neither a middleware name nor a list of them, or a throttle limit that is neither a number nor an attribute name, now throws an `InvalidArgumentException` naming the key (`null` keeps the default). **Upgrading from 1.x:** a tool route now answers 423 before the 2FA challenge, 409 to an unverified user when verification is on, 404 to a user the tool is hidden from, and 429 past the API throttle, which it shares with the rest of the API; with a custom `MARTIS_PATH` it moves to `/{MARTIS_PATH}/api/tools/{uriKey}`; a call that passes `['web', 'martis.auth']` keeps that stack and warns, so drop the argument; routes registered in `boot()` with that list keep it, unchecked, until switched to `ToolRoutes::middleware($this)`. +26 Pest. See [Tools → Tool routes and their middleware](docs/tools.md#tool-routes-and-their-middleware) and [Upgrading](docs/upgrading.md#tool-routes-run-the-martis-api-middleware).

### Changed

- **The docs MCP server's `martis_doc_list` describes each page as the docs site does**: the first paragraph after the title that is not a blockquote, heading, list, code fence or HTML, in plain text, up to 280 characters (`Martis\Support\DocDescription`). It gave the first non-heading line, blockquotes included, cut at 160, so a page that opens with a summary quote or a navigation line was listed by it; the listing is longer (about 9.8 KB for the package docs instead of 7.1 KB).
...
- **Row actions in relationship panels, as in Nova.** A `HasMany`, `HasManyThrough` or `MorphMany` panel now offers the related resource's inline actions (`showInline()`) on each row, laid out as on the index (an ungrouped action is an icon button, grouped ones sit in the "..." menu, now with `aria-haspopup` / `aria-expanded` and the focus moving into it), each disabled where the record may not run it. The panel runs the action on that record through the same modal and sends the relationship (`viaResource`, `viaResourceId`, `viaRelationship`, as Nova does), so only a record it reaches is run on; a `standalone()` action runs on no record, and an `openCreate` / `openDetail` / `openUpdate` answer opens its drawer over the page. Panel rows carry an `_actionAuthorization` map of the inline actions that run on a record, none when there is none. `BelongsToMany` / `MorphToMany` keep their pivot actions. Actions that cannot run stay visible and disabled on the row (Nova hides them there), and a `canRun()` grants nothing the policy refuses (in Nova 5 it replaces the policy); see [Differentials](docs/differentials.md). +2 Pest, +40 Vitest. See [Actions → In relationship panels](docs/actions.md#in-relationship-panels).
...
- **The loading spinners of the drawers and the two-factor wizard were invisible.** Tailwind's preflight is off, so `border-2` without a border style drew no ring: the create, update and detail drawers and the two-factor setup showed an empty box while loading. They carry `border-solid` now, and a Vitest walk of the components fails on any `animate-spin` ring without a border style. +1 Vitest.
- **Some accent surfaces were always indigo, and a theme's `--martis-dur-fast` missed some transitions.** The tool page header icon and a tool's master settings row read `--martis-brand-500`, which no stylesheet or theme defines, so its `#6366f1` fallback won whatever the theme or the `[data-accent]` choice; the selected option of a `BelongsTo` picker and its hover, the focus ring of the Trix link dialog, the links of a rich-text detail in light mode (and their hover), the sort carets of a relationship panel, the two-factor wizard's spinner and a `Sparkline` field without a `chartColor` used a fixed indigo too. They now read `--martis-accent` (or `--martis-focus-ring`). Eleven transitions read `--martis-dur-sm`, which is not defined either, through a `120ms` fallback; they now read `--martis-dur-fast` (the same 120ms), so they follow a theme that sets it. The components that read undefined tokens moved to defined ones too: `--martis-primary` to `--martis-accent` (the relation pickers' create links, the pivot panels' hover, the back link of the detail, create and update pages and the id link of each relationship panel row, which inherited the text colour and now show the accent), `--martis-card-bg` to `--martis-card` (two-factor wizard) and `--martis-input-height` to the density token `--martis-input-h` (lens dropdown, icon field). **Upgrading from 1.x:** a theme that set `--martis-dur-sm` or `--martis-brand-500` loses that effect, and `martis:theme:diff` lists them as *Unknown to package* (exit 2): set `--martis-dur-fast` and `--martis-accent` instead.
- **A scaffolded theme overrode the logo-height knobs of `.env`.** `martis:theme` wrote `--martis-brand-logo-height-auth: 48px` and `--martis-brand-logo-height-menu: 28px` on the theme's `:root`, so `MARTIS_BRAND_LOGO_HEIGHT_MENU` / `_AUTH` stopped applying once a theme was generated (the menu logo shrank from the 40px default to 28px). The stub now carries them commented out, as an override to uncomment. **Upgrading from 1.x:** in a theme generated before, delete those two lines unless you mean to override the `.env` values. `martis:theme:diff` also ignores a variable that only appears in a CSS comment now.
- **The scaffolded theme drifted from `martis.css`.** The stub's `--martis-font-sans` / `--martis-font-heading` lacked `'Inter var'`, its `--primary-color-text: #ffffff` pinned the PrimeReact text on accent fills to white, ignoring `--martis-accent-contrast`, and its light block left out the 16 light avatar colours. The stub now declares every value `martis.css` does, per mode, accent, density and motion, and `ThemeTokenDriftTest` compares them.
- **`martis:install` read the avatar column from a pipe.** The avatar column questions checked only `isInteractive()`, not the TTY every other prompt checks, so `yes | php artisan martis:install --with-profile` published a migration adding a `users.y` column and wrote `MARTIS_AVATAR_COLUMN=y`, and `docker compose exec -T` waited forever. They now ask only on a TTY, as the other prompts do, and take `profile_picture` otherwise (`--existing-avatar-column` needs `--avatar-column` there, and says so: "when the command runs without a terminal (no TTY or --no-interaction)"). The check (`canPrompt()`: interactive, a TTY, outside the tests) moved to a `Martis\Console\Concerns\AsksOnlyOnATerminal` trait, which `martis:sso` now uses for its role mapping questions too (it checked `isInteractive()` only). **If you already ran it through a pipe:** look for a `users.y` column (a migration adding it under `database/migrations`, and `MARTIS_AVATAR_COLUMN=y` in `.env`); roll it back or drop the column, set `MARTIS_AVATAR_COLUMN` to the column you want, and see [Upgrading](docs/upgrading.md#martisinstall-asks-only-on-a-terminal). +8 Pest.
- **The generators overwrote a customised file when their input was piped.** `martis:card`, `martis:field`, `martis:component`, `martis:tool` and `martis:theme` asked "already exists. Overwrite?" (default no) whenever Symfony's `isInteractive()` said so, which a pipe without `--no-interaction` does: `yes | php artisan martis:field Rating` answered "y" and rewrote the React component the developer had customised, and `docker compose exec -T` waited forever. `martis:invitations`, `martis:roles` and `martis:sso` asked "Run pending migrations now?" the same way. They all use `AsksOnlyOnATerminal` now: without a TTY each does what it does with `--no-interaction` (warns and leaves the file alone unless `--force`, fails on an existing theme, runs the migrations), `martis:user` asked for the email, the name and the password the same way, so `yes | php artisan martis:user` created an admin with the email, name and password `y`, its email already verified; without a terminal it now needs `--email` and `--password`, exits 1 naming the missing one and creates no user, and the name defaults to `Martis Admin`. `martis:agents` asked through Laravel Prompts, which answer their own defaults without a TTY (the MCP server wired, an existing guidelines file skipped) where `--no-interaction` does the opposite; it now behaves as with `--no-interaction`. A test fails when any command under `src/Console` asks (`ask`, `secret`, `confirm`, `choice`, `anticipate` or Laravel Prompts) without the trait. On a file that exists, every generator now prints an "already exists" error and exits 0, as Laravel's `GeneratorCommand` does (`martis:component` and `martis:theme` exited 1, and `martis:tool` printed "Aborting" and exited 0); nothing inside Martis reads those codes. `yes n | php artisan martis:invitations` (or `martis:roles`, `martis:sso`) answered "no" to the migrations and now runs them: pass `--no-migrate`. +28 Pest.
- **`martis:install --force` rewrote an application's own notifications and sessions migrations.** Any `*_create_notifications_table.php` / `*_create_sessions_table.php` was overwritten, those `make:notifications-table` / `make:session-table` created included. `--force` now rewrites only the ones Martis published, recognised by a sentence of the Martis stub's header (`Martis in-app notifications table.` since v0.8.0-beta, `Martis ships a "Browser sessions" profile section` since v1.30.0), and leaves the application's, including one whose comments merely mention Martis. **If an earlier `--force` rewrote yours:** compare the file with your version control and restore it. +6 Pest.
- **A custom `layout:shell` had no tooltips.** The global tooltip was mounted only inside the bundled layout presets (and the auth screens), so an override registered under `layout:shell` (or `martis.layout.components.shell`) showed no `data-pr-tooltip`. It is mounted once, above the shell switch, now. The `martis:component --type=topbar` stub uses `data-pr-tooltip` instead of `title` (no published stub uses `title`), the topnav search button's label is translated, and the one-of-many aggregate tile names its column to screen readers (`ofmany_aggregate_column`, in the three locales). +5 Pest, +9 Vitest.
- **Escape in a modal, a menu or a picker opened over a drawer closed the drawer too.** Pressing Escape in a delete confirmation, an action modal, the unsaved-changes prompt or the keyboard shortcuts help (`shift+?`) inside a drawer closed the modal and the drawer under it, and so did an Escape meant for the actions menu, a row's inline action menu, the lens menu, a Repeater's add menu, a `BelongsTo` / `MorphTo` / `Tag` / multi-select / icon picker, or a PrimeReact `Dropdown`, `MultiSelect`, `Calendar`, `AutoComplete`, `SplitButton`, `ColorPicker` or `OverlayPanel`, losing a Create or Update form. The drawer now closes on an Escape nothing else took: it decides in the capture phase, before the layers' listeners (a key press runs the microtasks between listeners, where React applies a layer's close, so a check made after one would find nothing open), and it leaves the Escape alone when it arrives `defaultPrevented`, while a modal holds its history lock, while a Martis popup is open or while a PrimeReact overlay is mounted. Every Martis modal and popup marks the Escape it takes with `preventDefault()`. The popups register through a new `useEscapeLayer(open, close)`, also on `@martis/runtime`, which closes the top popup only (the pickers above ignored Escape before). The keyboard shortcuts help takes the modal history lock, so the back button no longer dismisses it. A modal closed with no drawer mounted no longer leaves the history suppress flag set, which swallowed the first Back of the next drawer. Tests dispatch Escape as a browser does (one listener at a time, microtasks in between), also with the drawer's owner re-rendering while a layer is open; a test fails when a Martis popup that closes on an outside click does not register, or when a component's Escape handler does not mark the key taken. +29 Vitest. See [Components → Escape closes the top layer only](docs/components.md#escape-closes-the-top-layer-only-v200).
- **A one-record card's Delete could remove a record the user had not seen.** `DELETE` (and `PUT`) on `…/has-one/{relationship}` and `…/morph-one/{relationship}` carried no id, so the server wrote whatever record the relationship held at that moment: a record created or swapped in after the card loaded (a newer `latestByTimestamp()` / `latestOfMany()` record, a replaced `HasOne`, another record through a `HasOneThrough`) was deleted or updated instead of the one on screen. The card now sends the id it shows (`?relatedId=`); a write aimed at a record the relationship no longer holds answers 409 (`The record changed since the card loaded; reload to see it.`) and touches nothing, and the card reloads and shows the message in a toast. The card keeps the id it showed when Delete was clicked, so a refetch while the confirmation is open (the window regaining focus) cannot retarget it. The checks run 404 (a record the user may not view, whatever id is sent), 409 (the id names another record, before the policy, which is the current record's), 403 (the policy), 422 (no id), and the 409 and 422 messages are translated (`card_record_changed`, `card_related_id_required`). The delete confirmation (`DeleteModal`) now manages focus as a modal dialog: Cancel takes it on open, Tab stays inside, it returns to the control that opened it on close, and the dialog is named by its title (`aria-labelledby`, `aria-describedby`); after a delete the focus goes to the empty card's Create button, or its title, instead of the page body. **Upgrading from 1.x:** an API client or a custom card component that calls those endpoints must send `?relatedId=` (422 without it); see [Upgrading](docs/upgrading.md#a-one-record-card-write-names-its-record). +40 Pest, +35 Vitest. See [Relationships](docs/relationships.md#hasone).
- **The soft-delete filter did nothing on `BelongsToMany` and `MorphToMany` panels.** Their endpoints ignored `?trashed`, so *Only trashed* and *With trashed* listed the active records. They now apply it as the other panels do (the related resource must soft-delete and allow `canViewTrashed()`), as Nova's `BelongsToMany` panel does. +2 Pest.
- **An action whose selected records all failed to resolve answered "Done", and its row map ignored the policy.** `execute()` looked the ids up through `indexQuery()` with the default scopes, so when none of them resolved (outside the scope, a trashed row, a forged id) `handle()` ran on an empty collection and the run answered 200. It now answers 404 (`One or more selected resources could not be found.`), pivot actions included; when some resolve, the action runs on those, as in Nova. A run that names no record for an action that is not standalone answers 422 (`resources`), as a pivot run already did, and the records are looked up through the resource's `scopes()` too, as the index lists them: a resource that scoped its tenancy with `scopes()` resolved another tenant's ids. Trashed records are looked up too when the resource soft-deletes, since the index and the panels list them, and a `standalone()` action runs on no record whatever ids are sent. The per-row `_actionAuthorization` map of the index and the panels checked only `canRun()`, so an item stayed enabled when the `runAction` / `runDestructiveAction` policy refused it and the run answered 404; the map and the run now share one predicate, which asks the policy once per row (reusing the row's `_authorization` flags) rather than once per action. **Upgrading from 1.x:** see [Upgrading → Actions](docs/upgrading.md#actions-who-may-run-them-and-on-which-records). +26 Pest.
- **`martis:mcp-serve` over stdio answered nothing to a client that sends its requests and closes the pipe.** The bundled stdio transport buffers each response in a ReactPHP stream and, when STDIN closed, closed that stream at once: `close()` dropped what was still buffered, so the log said the responses were sent and stdout stayed empty, and it closed the STDOUT resource itself, so the console could exit 255 ("StreamOutput needs a stream"). `Martis\Mcp\Transport\FlushingStdioServerTransport` writes each response, all of it, before the next request is read, and leaves STDOUT open: piping `initialize` and `tools/list` now gives both responses on stdout and exit 0. A client that keeps the pipe open (every MCP client) was not affected. A response larger than the pipe (`martis_doc_read` of `fields` is about 190 KB) is written in parts as the client reads, and a SIGTERM or SIGINT while it waits for a client that stopped reading drops it and ends the server. +2 Pest.
- **`martis:mcp-serve --transport=http` hung, instead of exiting, when its port was in use.** The error was logged, but the signal listeners kept the ReactPHP loop, which runs itself at shutdown, waiting forever. The command stops the loop before it reports the error and exits 1. +1 Pest.
- **A SIGTERM or SIGINT that reached `martis:mcp-serve` before its loop ran was lost.** The server logs "is up and listening" (and over stdio opens its session) before it runs the loop, and `run()` clears a stop requested before it: over HTTP the process kept serving, and over stdio the transport closed itself while the session timer kept the process alive until a SIGKILL. A signal sent right after that log line can land there (a supervisor or a client that stops a server it just started). The command's handler now also stops the loop on its next tick, and it handles stdio too, which had only the transport's own. +2 Pest.
...
- **"extension load exceeded 5000 ms" was logged for extensions that loaded fine.** The loader raced every extension import against a 5 s timer but never cleared the timer, so five seconds after each page load the console warned `[martis] extension load exceeded 5000 ms; mounting without it` for every bundle in `MARTIS_EXTENSIONS`, a message meant only for a bundle that really did not load in time. The timer is cleared when the import settles. The loader moved to `resources/js/lib/extensionLoader.ts` and its test now exercises the real function instead of a copy of its logic, and a rejected import clears its timer too (+3 Vitest).
...
- The avatar palette is declared on `:root` and again on `html:not(.dark)` (the same values), in `martis.css` and the scaffolded theme: the theming guide and the stub's comments said it lived once on `:root`, so a theme that set it there recoloured the dark mode only. They now say to set both blocks.
- **The theme has 162 variables, not 94 (or 141).** The README, the docs index, the differentials and the preferences page said 94, and the theming guide "10 logical groups" and a total of 141, while a theme can define 162: the 160 `martis.css` gives a value, plus the two brand logo heights the config sets. The theming guide's Variable Reference now lists all 162 in 19 groups with their dark and light values from `martis.css`, says to set the short typography names (the package CSS does not read the `--martis-font-size-*` / `--martis-font-weight-*` / `--martis-line-height-*` aliases), names the inline layout variables and the tooltip hooks it does not count, and the motion section names the five durations that exist. `tests/Unit/ThemeTokenDriftTest.php` fails when `martis.css`, the stub and the reference disagree (names, the stub's and the reference's values, group counts, the count table, the number of groups), when another page states another total, and when the package CSS or a component reads a `--martis-*` variable nothing defines. +7 Pest.
- Every docs page opens with a paragraph that describes it, short enough for the docs site to use whole. The site takes a page's description (its search excerpt and meta description) from its first paragraph that is not a blockquote, heading, list, code fence or HTML, and keeps 280 characters of it. That paragraph was a line of navigation on the SSO page, a table on the preferences page, a generic or partial sentence on the relationships and in-app Tool boot pages, and ran past 280 characters on the authorization, cache, generator stubs, impersonation, invitations and repeater pages; the index, the roles page and the v1 roadmap, which the site does not publish from these files, got the same treatment. `Martis\Support\DocDescription` applies the site's rule, and `tests/Unit/DocsPageDescriptionTest.php` holds every page to it (no backslash either: the site writes the description into a YAML string that does not escape one). Its first sentence also runs 60 to 160 characters, the part a search result or a link preview shows: twelve pages opened with a first sentence under 60 characters (the resources, configuration, dashboards, notifications, overrides, quick start and sticky views pages) or past 160 (the index, agent guidelines, authentication, differentials and Tool fields pages), and none may contain an em dash. +4 Pest.
- The installation guide and the troubleshooting page name the avatar migration `martis:install` writes (`*_add_martis_profile_picture_column_to_users_table.php`; the short name is the `martis-avatar-migration` tag's), drop the redundant `martis-preferences-migration` step of the manual install (`martis-migrations` includes it), and say that a piped or TTY-less install takes the `profile_picture` column; `i18n.md` says a missing preferences table falls back to the defaults instead of throwing, and uses a valid namespace name (`app_actions`).
...
- **The components' theme-variable reads are checked with the TypeScript parser.** `ThemeTokenDriftTest` scanned the components with a comment stripper, which an apostrophe in JSX text, a quote inside a regex literal or `//` in JSX text threw off; `resources/js/themeTokenReads.test.ts` walks their string literals, template pieces and JSX text with `ts.createSourceFile` instead, and the Pest test keeps `martis.css`, the stub and the reference. Its value parser reads nested rules too, so the stub's `@media (prefers-reduced-motion: reduce)` block is compared with `martis.css`'s. +2 Vitest.
- **The `martis:mcp-serve` transport tests waited on fixed clocks.** The stdio test gave the subprocess 5s to finish, the readiness gate 8s, the SIGTERM check 10s, the HTTP calls 2-5s, and the `0.0.0.0` warnings were read after a 200ms sleep, so on a loaded machine (parallel suites, the Docker wrapper) the file failed whole or in part while CI passed. Every wait now ends when its condition holds (the port answers, the marker is logged, the process exits) under one ceiling, `MARTIS_TEST_PROCESS_TIMEOUT` (30s by default), and a failure says why: how long it waited, whether the process was running or its exit code, and its stderr and stdout. The stdio server is tracked for the forced cleanup too, and a finished server is stopped with SIGTERM under the same ceiling (a 2s stop escalated to SIGKILL, which leaves the skeleton's `.env`). A SIGTERM is sent only after an HTTP round trip (a `ping`), since the "is up and listening" marker is logged before the loop runs; the HTTP calls report curl's error and the process's output when they fail; the `0.0.0.0` checks match the whole warning sentences; the stdio test proves the `tools/list` response on stdout and exit 0 (it read the tool names from the log on stderr, which hid the empty stdout above); a new test occupies the port and expects exit 1 with `Address already in use`. A SIGTERM sent from outside right after the marker can land before `run()` (the review of #258 measured 5 of 30 HTTP runs under load and 12 of 12 stdio runs in fresh containers), but whether it does is up to the machine, so the tests put it there: one drives the handler in process (a signal delivered before `run()` must end the loop; it checks that a SIGTERM handler is installed before it signals, since an unhandled one would end the test run itself wherever Pest is not PID 1, and puts back the handlers the process had), and a spawned stdio server's loop signals its own process as it starts to run (`tests/Support/mcp-serve-signal-on-run.php`, loaded with `auto_prepend_file`). The test of a SIGTERM while an answer waits runs the server under `proc_open()`, since a `Process` drains stdout whenever it is polled. An invalid `MARTIS_TEST_PROCESS_TIMEOUT` (`abc`, `0`, `-5`) throws, naming the variable, instead of falling back to 30s, and `scripts/test.sh` passes it into the container. +3 Pest.
...
- **A create through a Through relationship filed the record under another parent.** The `has-many` and `has-one` stores wrote the parent's key into the related record's key to the intermediate model (a project's `client_id` under a team's `hasManyThrough(Project::class, Client::class)`), which files the record under whichever intermediate has that id, or fails on the foreign key when none has. It was reachable through a plain `HasMany` field declared on a `hasManyThrough` relationship, which offers Create by default, through a `HasManyThrough` field by calling the endpoint, and through a plain `HasOne` / `HasOneOfMany` field declared on a `hasOneThrough` one. Those creates now answer 403 (`Records cannot be created through a hasManyThrough relationship.` / `… a hasOneThrough relationship.`). A `HasManyThrough` field that calls `canCreate(true)` keeps creating: it is the escape hatch for an app that sets that key itself (in the related resource's `beforeSave()` or an observer), and without that its creates are filed the same way. A plain `HasMany` field on a `hasManyThrough` relationship, and a `HasOne` / `HasOneOfMany` field on a `hasOneThrough` one, still show Create (`canCreate()` defaults to `true` there) and the create now fails with that 403: declare them as `HasManyThrough` / `HasOneThrough`, or call `->canCreate(false)`. If a panel offered Create on a Through relationship, check the second key of the records created from it. +4 Pest.
- **A `HasManyThrough` panel showed, and acted on, another record's id.** The `has-many` index paginated the relation's bare query, which on a `hasManyThrough` selects every column of the join, so the intermediate record's `id`, timestamps and `deleted_at` overwrote the listed record's. A row showed another record's id, and its View, Restore and Force delete (Edit and Delete too, where the panel offers them) targeted that record: a Force delete could permanently remove an active record of another parent. A trashed record came back with `deleted_at: null` when the intermediate model soft-deletes. The index now paginates through the relation, which selects only the related table's columns, so the panel's sort needs no qualified column (an alias such as a `withCount` column keeps sorting), except a JSON path (`meta->code`), which the `hasManyThrough`, `belongsToMany` and `morphToMany` panels qualify. Those three panels also qualify the columns they search by, which the intermediate or the pivot made ambiguous when it has the same column; the other panels and the resource index keep their search columns as written, since their relation or `indexQuery()` may join the table a searchable field reads. A `belongsToMany`, `morphToMany` or `hasManyThrough` relation that joins another table itself can therefore no longer be searched by a column only that table has. If you use a `HasManyThrough` panel, check the records restored or force-deleted from it. +8 Pest.
- **The Delete of a `HasOneOfMany` / `MorphOneOfMany` card removed another record than the one it showed.** With `latestByTimestamp()` or `oldestByTimestamp()` over a `hasMany` / `morphMany`, the card showed the record that order picks, but `DELETE …/has-one/{relationship}` (and `…/morph-one/{relationship}`), and a `PUT` sent straight to those routes, wrote `$relation->first()`, the first row in storage order: the card's Delete on the newest note deleted the oldest one. The card's Edit opens the related resource's own edit page with the shown id, so it was not affected. Show, update and destroy now read the same record through the relation, and the related resource's policies run on it. An Eloquent `latestOfMany()` / `ofMany()` relation was not affected. If you use these cards, check the records deleted from them: the deleted row may be a different one than the card showed (the first row stored).
- **A one-of-many card over a `hasOneThrough` showed and counted another parent's records.** With `latestByTimestamp()` or `oldestByTimestamp()`, the card showed the intermediate record's id, so its Edit opened a record of another parent. With those or with an Eloquent one-of-many relation (`latestOfMany()`, `oldestOfMany()`, `ofMany()`), its "1 of N" count and `aggregateVia()` tile matched the intermediate table's ids, so they counted and summed another parent's rows. The card now reads the record through the relation. The count and the tile go through the relation itself, so on a `hasMany`, `morphMany` or through relation they also honour the constraints written into it (`->where('paid', true)`) and a custom local key on `hasOne` / `morphOne`. On an Eloquent one-of-many relation (`latestOfMany()`, `ofMany()`) they cover every related row of the parent: the constraints written into that relation do not apply to them.
- **`latestByTimestamp()` / `oldestByTimestamp()` break a timestamp tie on the primary key**, as Eloquent's `latestOfMany()` does, so every database picks the same record for the card and its writes: with equal timestamps, latest now shows the highest id and oldest the lowest. The related model's `created_at` / `updated_at` are qualified with its table, so the default column works over a through relation that shares it; another column is used as given (qualify one that both tables have). +24 Pest.
