...
2. Clear the schema cache after updating: `php artisan martis:cache:clear schema` (it never expires by default).
3. Flip every associative array written label first: `['Draft' => 'draft']` becomes `['draft' => 'Draft']`, `pluck('id', 'name')` becomes `pluck('name', 'id')`.
4. Turn lists into maps: a list is keyed 0, 1, 2…, as in Nova, so `['draft', 'published']` now stores `0` and `1`. Pass `array_combine($values, $values)` when each value is its own label, or pass an enum class.
5. Rewrite MultiSelect groups in Nova's format: `['Backend' => ['PHP' => 'php']]` becomes `['php' => ['label' => 'PHP', 'group' => 'Backend']]`. The old format throws an `InvalidArgumentException` naming the field and the option.
6. Replace `optionsFromMap()` with `options()`.
7. Return `[value => label]` from `searchOptionsUsing()` closures: a list stores positions that change with the search term.
...
`grep -rnF -e '->options(' -e 'optionsFromMap(' -e 'searchOptionsUsing(' app/` finds the calls; filters are not affected. See [Upgrading](docs/upgrading.md).

### Changed

- **BREAKING: `Select` and `MultiSelect` read `options()` in Nova's order, `[value => label]`**, lists included (keyed 0, 1, 2…). `searchOptionsUsing()` returns the same shapes in the same order. The two fields share one normaliser (`Martis\Fields\Concerns\HasChoiceOptions`), so they accept the same forms. +9 Pest. See [Fields → Option order](docs/fields.md#option-order).
