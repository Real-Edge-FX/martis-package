...
- **`vendor/bin/pest --parallel` gives each worker its own copy of the testbench skeleton.** The workers shared the skeleton under `vendor/` that the suite writes into (`martis:install` publishes assets, config and migrations; the generators write `app/Martis` classes), so one worker read the files another was writing or deleting: 24 to 27 failures a run ("Martis frontend assets did not publish successfully", `Failed opening required …/config/…`, `strict_types declaration must be the very first statement`) and, now and then, a crashed worker. `TestCase::applicationBasePath()` copies the skeleton to a temporary directory on a worker's first test (paratest sets `TEST_TOKEN` in each worker) and removes the copy when the worker exits; `McpServeCommandTransportTest` runs `vendor/bin/testbench` in that same copy (`APP_BASE_PATH`). A sequential run, CI's included, keeps the shared skeleton. +1 Pest (drives the worker branch in a subprocess, since CI runs sequentially). Three consecutive parallel runs: 3187 passed, 0 failed.
- **A local `vendor/bin/pest` runs under a stock `php.ini`, and `scripts/test.sh` pins nothing CI does not.** One process runs the whole suite, which peaks at about 170 MB (83 MB of it is loading the tests), while a stock `php.ini` allows 128M: a plain local run died part-way (`Allowed memory size of 134217728 bytes exhausted`), and CI passed because setup-php runs PHP with no memory limit. `phpunit.xml` now sets `memory_limit` to 1G. `scripts/test.sh` drops its `-d memory_limit=1G` and its `CACHE_STORE=array` / `SESSION_DRIVER=array` pins: the "cookies on null" errors the pins hid came from the `.env` left in the testbench skeleton the container mounts, which the suite now ignores.
