...
- **Action events stored `original` / `changes` double-encoded (high).** `ExecuteAction` passed `json_encode($diff)` for the `original` and `changes` columns, but `ActionEvent` casts both to `'array'` — so Eloquent encoded the already-encoded string again. Reads returned a JSON string instead of an array, breaking any consumer of the recorded before/after diff. The raw arrays are now passed; the cast encodes once.
