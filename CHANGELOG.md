...
- **Tests that edit the testbench skeleton's own files put them back as they found them; the suite pins the drivers it assumes and cleans up only its own temporary files.** `Martis\Tests\Support\SkeletonSnapshot` captures skeleton paths (contents and modification times, directories, symlinks as links) and restores them exactly. `ConsoleCommandsTest` restores `bootstrap/providers.php` and the host `MartisServiceProvider` stub (its cleanup rewrote the one and a test replaced it wholesale), `RolesScaffoldCommandTest` restores `app/Models`, `app/Providers`, `app/Policies`, `app/Martis/Resources` and `database/seeders` (its hooks deleted them, `app/Providers` included), and `McpServeCommandTransportTest` restores the skeleton's `vendor` link, which a killed `vendor/bin/testbench` left behind; each takes its snapshot before its first test and restores it after its last. The artifacts `martis:install` and the generators publish are still removed after each test, as before. `SsoMakeCommandTest` puts `.env`, `.env.example`, `config/martis.php`, `config/permission.php` and `app/Providers/AppServiceProvider.php` back byte for byte and deletes the migrations the command published: it stripped only the lines the command added, so the skeleton's `.env.example` grew a few bytes a run, and the azure block written into `config/martis.php` reached later tests. Its listener test, skipped because the skeleton has no `AppServiceProvider`, now writes the one a fresh Laravel app has and runs. `ConsoleCommandsTest` no longer rewrites the skeleton's `.env`: `martis:install` writes no environment file while unit tests run, now a test. `TestCase` pins `app.debug` (false), `cache.default` and `session.driver` (`array`) and `queue.default` (`sync`), the skeleton defaults, since a variable exported in the shell still reaches the config. `PublishAssetsCommandTest` removes the temporary directories it created, not every `martis-pub-*` one (another suite's too), and `MigrationStubsUserKeyTest` and `McpServeCommandTest` no longer leave an empty `tempnam()` placeholder behind on every run. +2 Pest, and one test no longer skipped.
