...
- **The `schema` cache layer expires after a day by default (`MARTIS_CACHE_SCHEMA_TTL=1440`), and a cache key over 191 characters is hashed.** Now that every key carries the installed version, each upgrade leaves the previous version's entries behind, and with no TTL the file and database stores kept them forever. A `config/martis.php` published before v2.0 keeps `null`: set `MARTIS_CACHE_SCHEMA_TTL=1440`, and use a dedicated cache store if you want `cache:clear` to remove old Martis entries only. A key longer than 191 characters (a `utf8mb4`-indexed database cache column; memcached stops at 250 bytes) is written as `martis:cache:{type}:h:{sha256}` of the whole key. +4 Pest. See [Cache → Invalidation](docs/cache.md#invalidation).
